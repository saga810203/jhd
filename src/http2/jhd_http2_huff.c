...
typedef struct {
    ...
    u_char  sym;
    u_char  ending;
} jhd_http2_huff_decode_code_t;

static jhd_http2_huff_decode_code_t  ngx_http_v2_huff_decode_codes[256][16] =
{
    /* 0 */
    {
...
int jhd_http2_huff_decode(u_char *src, uint16_t src_len, u_char *dst,uint16_t dst_len){
   ...
   (void*) dst_len;
    ...
    while (src != end) {
        ch = *src;
        ++src;
        code = ngx_http_v2_huff_decode_codes[state][ch>>4];
        if (code.next == state) {
        	jhd_err = JHD_HTTP2_ERROR_HUFF_DECOE;
            return JHD_ERROR;
        }
        if (code.emit) {
...
//        ending = code.ending;
        state = code.next;
        code = ngx_http_v2_huff_decode_codes[state][ch & 0xf];
        if (code.next == state) {
        	jhd_err = JHD_HTTP2_ERROR_HUFF_DECOE;
            return JHD_ERROR;
        }
