...
int jhd_add_pool_config(u_int16_t size, u_int32_t page_size,
	...
	if (new_pool) {
		new_pool->size = size;
		new_pool->real_size = size + sizeof(u_int16_t);
		if (new_pool->real_size < (sizeof(void*) + sizeof(u_int16_t))) {
			new_pool->real_size = (sizeof(void*) + sizeof(u_int16_t));
		}
...
int jhd_pool_init() {
	...
	if ((!jhd_max_pool) || (jhd_max_pool->size < JHD_MAX_POOL_SIZE)) {
		...
		jhd_max_pool->real_size = JHD_MAX_POOL_SIZE + sizeof(u_int16_t);
...
void jhd_free(void* ptr){
	jhd_pool_t  *pool;
	u_int16_t size = *((u_int16_t*)(((u_char*)ptr)-2));
