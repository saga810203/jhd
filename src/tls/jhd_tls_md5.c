...
void jhd_tls_md5_free( jhd_tls_md5_context *ctx )
{
    ...
    jhd_tls_platform_zeroize( ctx, sizeof( jhd_tls_md5_context ) );
}
...
void jhd_tls_md5_starts( jhd_tls_md5_context *ctx )
{
...
#if !defined(JHD_TLS_DEPRECATED_REMOVED)
void jhd_tls_md5_process( jhd_tls_md5_context *ctx,
                          const unsigned char data[64] )
{
    jhd_tls_internal_md5_process( ctx, data );
}
#endif
                            ...
                            size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        if( ( ret = jhd_tls_internal_md5_process( ctx, ctx->buffer ) ) != 0 )
            return( ret );
    ...
    while( ilen >= 64 )
    {
        if( ( ret = jhd_tls_internal_md5_process( ctx, input ) ) != 0 )
            return( ret );
...
#if !defined(JHD_TLS_DEPRECATED_REMOVED)
void jhd_tls_md5_update( jhd_tls_md5_context *ctx,
                         const unsigned char *input,
                         size_t ilen )
{
                            ...
                            unsigned char output[16] )
{
    ...
    if( ( ret = jhd_tls_md5_update_ret( ctx, md5_padding, padn ) ) != 0 )
            return( ret );

    if( ( ret = jhd_tls_md5_update_ret( ctx, msglen, 8 ) ) != 0 )
            return( ret );
...
#if !defined(JHD_TLS_DEPRECATED_REMOVED)
void jhd_tls_md5_finish( jhd_tls_md5_context *ctx,
                         unsigned char output[16] )
{
                     ...
                     unsigned char output[16] )
{
    ...
    if( ( ret = jhd_tls_md5_starts_ret( &ctx ) ) != 0 )
        goto exit;

    if( ( ret = jhd_tls_md5_update_ret( &ctx, input, ilen ) ) != 0 )
        goto exit;

    if( ( ret = jhd_tls_md5_finish_ret( &ctx, output ) ) != 0 )
        goto exit;

exit:
    jhd_tls_md5_free( &ctx );

    return( ret );
}

#if !defined(JHD_TLS_DEPRECATED_REMOVED)
void jhd_tls_md5( const unsigned char *input,
                  size_t ilen,
                  unsigned char output[16] )
{
    jhd_tls_md5_ret( input, ilen, output );
}
#endif
...
int jhd_tls_md5_self_test( int verbose )
{
    ...
    for( i = 0; i < 7; i++ )
    {
        ...
        ret = jhd_tls_md5_ret( md5_test_buf[i], md5_test_buflen[i], md5sum );
        if( ret != 0 )
            goto fail;
