                             ...
                             const unsigned char *key, unsigned int keybits )
{
    ...
    if( keybits != 128 && keybits != 192 && keybits != 256 )
        return( JHD_TLS_ERR_ARIA_INVALID_KEY_LENGTH );
    ...
    jhd_tls_platform_zeroize( w, sizeof( w ) );
                             ...
                             const unsigned char *key, unsigned int keybits )
{
    ...
    ret = jhd_tls_aria_setkey_enc( ctx, key, keybits );
    if( ret != 0 )
        return( ret );
...
{
    memset( ctx, 0, sizeof( jhd_tls_aria_context ) );
}
...
void jhd_tls_aria_free( jhd_tls_aria_context *ctx )
{
    ...
    jhd_tls_platform_zeroize( ctx, sizeof( jhd_tls_aria_context ) );
}
...
{
    int i;
    unsigned char temp[JHD_TLS_ARIA_BLOCKSIZE];

    if( length % JHD_TLS_ARIA_BLOCKSIZE )
        return( JHD_TLS_ERR_ARIA_INVALID_INPUT_LENGTH );

    if( mode == JHD_TLS_ARIA_DECRYPT )
    {
        while( length > 0 )
        {
            memcpy( temp, input, JHD_TLS_ARIA_BLOCKSIZE );
            jhd_tls_aria_crypt_ecb( ctx, input, output );

            for( i = 0; i < JHD_TLS_ARIA_BLOCKSIZE; i++ )
                output[i] = (unsigned char)( output[i] ^ iv[i] );

            memcpy( iv, temp, JHD_TLS_ARIA_BLOCKSIZE );

            input  += JHD_TLS_ARIA_BLOCKSIZE;
            output += JHD_TLS_ARIA_BLOCKSIZE;
            length -= JHD_TLS_ARIA_BLOCKSIZE;
        }
    }
    else
    {
        while( length > 0 )
        {
            ...
            jhd_tls_aria_crypt_ecb( ctx, output, output );
            memcpy( iv, output, JHD_TLS_ARIA_BLOCKSIZE );
                               ...
                               unsigned char *output )
{
    ...
    if( mode == JHD_TLS_ARIA_DECRYPT )
    {
        ...
        {
            if( n == 0 )
                jhd_tls_aria_crypt_ecb( ctx, iv, iv );
                            ...
                            unsigned char *output )
{
    ...
    {
        if( n == 0 ) {
            jhd_tls_aria_crypt_ecb( ctx, nonce_counter,
                                stream_block );

            for( i = JHD_TLS_ARIA_BLOCKSIZE; i > 0; i-- )
...
static const uint8_t aria_test1_ecb_pt[JHD_TLS_ARIA_BLOCKSIZE] =            // plaintext
{
    0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,     // same for all
    0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF      // key sizes
};

static const uint8_t aria_test1_ecb_ct[3][JHD_TLS_ARIA_BLOCKSIZE] =         // ciphertext
{
 ...
 */
    ...
    defined(JHD_TLS_CIPHER_MODE_CTR))
...
#if (defined(JHD_TLS_CIPHER_MODE_CBC) || defined(JHD_TLS_CIPHER_MODE_CFB))
static const uint8_t aria_test2_iv[JHD_TLS_ARIA_BLOCKSIZE] =
{
...
{
    int i;
    uint8_t blk[JHD_TLS_ARIA_BLOCKSIZE];
    jhd_tls_aria_context ctx;

#if (defined(JHD_TLS_CIPHER_MODE_CFB) || defined(JHD_TLS_CIPHER_MODE_CTR))
    size_t j;
#endif

#if (defined(JHD_TLS_CIPHER_MODE_CBC) || \
     defined(JHD_TLS_CIPHER_MODE_CFB) || \
     defined(JHD_TLS_CIPHER_MODE_CTR))
    uint8_t buf[48], iv[JHD_TLS_ARIA_BLOCKSIZE];
#endif
    ...
    for( i = 0; i < 3; i++ )
    {
        ...
        jhd_tls_aria_setkey_enc( &ctx, aria_test1_ecb_key, 128 + 64 * i );
        jhd_tls_aria_crypt_ecb( &ctx, aria_test1_ecb_pt, blk );
        if( memcmp( blk, aria_test1_ecb_ct[i], JHD_TLS_ARIA_BLOCKSIZE ) != 0 )
            ARIA_SELF_TEST_IF_FAIL;

        /* test ECB decryption */
        if( verbose )
            printf( "  ARIA-ECB-%d (dec): ", 128 + 64 * i );
        jhd_tls_aria_setkey_dec( &ctx, aria_test1_ecb_key, 128 + 64 * i );
        jhd_tls_aria_crypt_ecb( &ctx, aria_test1_ecb_ct[i], blk );
        if( memcmp( blk, aria_test1_ecb_pt, JHD_TLS_ARIA_BLOCKSIZE ) != 0 )
            ARIA_SELF_TEST_IF_FAIL;
    }
    if( verbose )
        printf( "\n" );
    ...
    for( i = 0; i < 3; i++ )
    {
        ...
        jhd_tls_aria_setkey_enc( &ctx, aria_test2_key, 128 + 64 * i );
        memcpy( iv, aria_test2_iv, JHD_TLS_ARIA_BLOCKSIZE );
        memset( buf, 0x55, sizeof( buf ) );
        jhd_tls_aria_crypt_cbc( &ctx, JHD_TLS_ARIA_ENCRYPT, 48, iv,
            aria_test2_pt, buf );
        ...
        jhd_tls_aria_setkey_dec( &ctx, aria_test2_key, 128 + 64 * i );
        memcpy( iv, aria_test2_iv, JHD_TLS_ARIA_BLOCKSIZE );
        memset( buf, 0xAA, sizeof( buf ) );
        jhd_tls_aria_crypt_cbc( &ctx, JHD_TLS_ARIA_DECRYPT, 48, iv,
    ...
    for( i = 0; i < 3; i++ )
    {
        ...
        memset( buf, 0x55, sizeof( buf ) );
        j = 0;
        jhd_tls_aria_crypt_cfb128( &ctx, JHD_TLS_ARIA_ENCRYPT, 48, &j, iv,
            aria_test2_pt, buf );
        ...
        memset( buf, 0xAA, sizeof( buf ) );
        j = 0;
        jhd_tls_aria_crypt_cfb128( &ctx, JHD_TLS_ARIA_DECRYPT, 48, &j,
    ...
    for( i = 0; i < 3; i++ )
    {
        ...
        jhd_tls_aria_setkey_enc( &ctx, aria_test2_key, 128 + 64 * i );
        memset( iv, 0, JHD_TLS_ARIA_BLOCKSIZE );                    // IV = 0
        memset( buf, 0x55, sizeof( buf ) );
        j = 0;
        jhd_tls_aria_crypt_ctr( &ctx, 48, &j, iv, blk,
            aria_test2_pt, buf );
