 ...
 *  http://homes.esat.kuleuven.be/~bosselae/jhd_tls_ripemd160.html
...
void jhd_tls_ripemd160_free( jhd_tls_ripemd160_context *ctx )
{
    ...
    jhd_tls_platform_zeroize( ctx, sizeof( jhd_tls_ripemd160_context ) );
}
...
void jhd_tls_ripemd160_starts( jhd_tls_ripemd160_context *ctx )
{
...
#if !defined(JHD_TLS_DEPRECATED_REMOVED)
void jhd_tls_ripemd160_process( jhd_tls_ripemd160_context *ctx,
                                const unsigned char data[64] )
{
    jhd_tls_internal_ripemd160_process( ctx, data );
}
#endif
                                  ...
                                  size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        if( ( ret = jhd_tls_internal_ripemd160_process( ctx, ctx->buffer ) ) != 0 )
            return( ret );
    ...
    while( ilen >= 64 )
    {
        if( ( ret = jhd_tls_internal_ripemd160_process( ctx, input ) ) != 0 )
            return( ret );
...
#if !defined(JHD_TLS_DEPRECATED_REMOVED)
void jhd_tls_ripemd160_update( jhd_tls_ripemd160_context *ctx,
                               const unsigned char *input,
                               size_t ilen )
{
                                  ...
                                  unsigned char output[20] )
{
    ...
    ret = jhd_tls_ripemd160_update_ret( ctx, ripemd160_padding, padn );
    if( ret != 0 )
        return( ret );

    ret = jhd_tls_ripemd160_update_ret( ctx, msglen, 8 );
    if( ret != 0 )
        return( ret );
...
#if !defined(JHD_TLS_DEPRECATED_REMOVED)
void jhd_tls_ripemd160_finish( jhd_tls_ripemd160_context *ctx,
                               unsigned char output[20] )
{
                           ...
                           unsigned char output[20] )
{
    ...
    if( ( ret = jhd_tls_ripemd160_starts_ret( &ctx ) ) != 0 )
        goto exit;

    if( ( ret = jhd_tls_ripemd160_update_ret( &ctx, input, ilen ) ) != 0 )
        goto exit;

    if( ( ret = jhd_tls_ripemd160_finish_ret( &ctx, output ) ) != 0 )
        goto exit;

exit:
    jhd_tls_ripemd160_free( &ctx );

    return( ret );
}

#if !defined(JHD_TLS_DEPRECATED_REMOVED)
void jhd_tls_ripemd160( const unsigned char *input,
                        size_t ilen,
                        unsigned char output[20] )
{
    jhd_tls_ripemd160_ret( input, ilen, output );
}
#endif
...
int jhd_tls_ripemd160_self_test( int verbose )
{
    ...
    for( i = 0; i < TESTS; i++ )
    {
        ...
        ret = jhd_tls_ripemd160_ret( ripemd160_test_str[i],
