...
static void ssl_write_hostname_ext( jhd_tls_ssl_context *ssl,
                                    unsigned char *buf,
                                    size_t *olen )
{
    unsigned char *p = buf;
    ...
    *olen = 0;
    ...
    JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, adding server name extension: %s",
                   ssl->hostname ) );
                                         ...
                                         size_t *olen )
{
    ...
    *olen = 0;
    ...
    JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, adding renegotiation extension" ) );
...
static void ssl_write_signature_algorithms_ext( jhd_tls_ssl_context *ssl,
                                                unsigned char *buf,
                                                size_t *olen )
{
    unsigned char *p = buf;
    ...
    const int *md;
#if defined(JHD_TLS_RSA_C) || defined(JHD_TLS_ECDSA_C)
    unsigned char *sig_alg_list = buf + 6;
#endif

    *olen = 0;

    if( ssl->conf->max_minor_ver != JHD_TLS_SSL_MINOR_VERSION_3 )
        return;

    JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, adding signature_algorithms extension" ) );

    for( md = ssl->conf->sig_hashes; *md != JHD_TLS_MD_NONE; md++ )
    {
...
static void ssl_write_supported_elliptic_curves_ext( jhd_tls_ssl_context *ssl,
                                                     unsigned char *buf,
                                                     size_t *olen )
{
    unsigned char *p = buf;
...
#if defined(JHD_TLS_ECP_C)
    for( grp_id = ssl->conf->curve_list; *grp_id != JHD_TLS_ECP_DP_NONE; grp_id++ )
#else
                                        ...
                                        size_t *olen )
{
    ...
    *olen = 0;
    ...
    JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, adding ecjpake_kkpp extension" ) );
    ...
    *p++ = (unsigned char)( ( JHD_TLS_TLS_EXT_ECJPAKE_KKPP      ) & 0xFF );
    ...
    {
        JHD_TLS_SSL_DEBUG_MSG( 3, ( "generating new ecjpake parameters" ) );
        ...
        ssl->handshake->ecjpake_cache = jhd_tls_calloc( 1, kkpp_len );
        if( ssl->handshake->ecjpake_cache == NULL )
        {
            JHD_TLS_SSL_DEBUG_MSG( 1, ( "allocation failed" ) );
            return;
        }
    ...
    {
        JHD_TLS_SSL_DEBUG_MSG( 3, ( "re-using cached ecjpake parameters" ) );
                                          ...
                                          unsigned char *buf, size_t *olen )
{
    ...
    *olen = 0;
    ...
    if( ssl->conf->trunc_hmac == JHD_TLS_SSL_TRUNC_HMAC_DISABLED )
    {
        return;
    }
                                       ...
                                       unsigned char *buf, size_t *olen )
{
    ...
    *olen = 0;
    ...
    if( ssl->conf->encrypt_then_mac == JHD_TLS_SSL_ETM_DISABLED ||
        ssl->conf->max_minor_ver == JHD_TLS_SSL_MINOR_VERSION_0 )
    {
        return;
    }

    JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, adding encrypt_then_mac "
                        "extension" ) );
                                       ...
                                       unsigned char *buf, size_t *olen )
{
    ...
    *olen = 0;
    ...
    if( ssl->conf->extended_ms == JHD_TLS_SSL_EXTENDED_MS_DISABLED ||
        ssl->conf->max_minor_ver == JHD_TLS_SSL_MINOR_VERSION_0 )
    {
        return;
    }

    JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, adding extended_master_secret "
                        "extension" ) );
                                          ...
                                          unsigned char *buf, size_t *olen )
{
    ...
    *olen = 0;
    ...
    if( ssl->conf->session_tickets == JHD_TLS_SSL_SESSION_TICKETS_DISABLED )
    {
        return;
    }

    JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, adding session ticket extension" ) );
    ...
    *olen = 4;
    ...
    JHD_TLS_SSL_DEBUG_MSG( 3, ( "sending session ticket of length %d", tlen ) );
 ...
 */
static int ssl_generate_random( jhd_tls_ssl_context *ssl )
{
    int ret;
    ...
    *p++ = (unsigned char)( t       );
    ...
    JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, current time: %lu", t ) );
#else
 ...
 */
static int ssl_validate_ciphersuite( const jhd_tls_ssl_ciphersuite_t * suite_info,
                                     const jhd_tls_ssl_context * ssl,
                                     int min_minor_ver, int max_minor_ver )
{
    ...
    if( suite_info->key_exchange == JHD_TLS_KEY_EXCHANGE_ECJPAKE &&
            jhd_tls_ecjpake_check( &ssl->handshake->ecjpake_ctx ) != 0 )
        return( 1 );
#endif
...
static int ssl_write_client_hello( jhd_tls_ssl_context *ssl )
{
    ...
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "configured max major version is invalid, "
                            "consider using jhd_tls_ssl_config_defaults()" ) );
    ...
    jhd_tls_ssl_write_version( ssl->conf->max_major_ver, ssl->conf->max_minor_ver,
                       ssl->conf->transport, p );
    p += 2;

    JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, max version: [%d:%d]",
                   buf[4], buf[5] ) );

    if( ( ret = ssl_generate_random( ssl ) ) != 0 )
    {
        JHD_TLS_SSL_DEBUG_RET( 1, "ssl_generate_random", ret );
        return( ret );
    }
    ...
    *p++ = (unsigned char) n;
    ...
    if( ssl->conf->transport == JHD_TLS_SSL_TRANSPORT_DATAGRAM )
    {
        ...
        {
            JHD_TLS_SSL_DEBUG_MSG( 3, ( "no verify cookie to send" ) );
            *p++ = 0;
        }
        else
        {
    ...
    for( i = 0; ciphersuites[i] != 0; i++ )
    {
        ...
        JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, add ciphersuite: %04x",
                                    ciphersuites[i] ) );
    ...
    *q++ = (unsigned char)( n << 1 );
    ...
    if( offer_compress )
    {
        JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, compress len.: %d", 2 ) );
        JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, compress alg.: %d %d",
                            JHD_TLS_SSL_COMPRESS_DEFLATE, JHD_TLS_SSL_COMPRESS_NULL ) );

        *p++ = 2;
        *p++ = JHD_TLS_SSL_COMPRESS_DEFLATE;
        *p++ = JHD_TLS_SSL_COMPRESS_NULL;
    }
    else
    {
        JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, compress len.: %d", 1 ) );
        JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, compress alg.: %d",
                            JHD_TLS_SSL_COMPRESS_NULL ) );
    ...
    JHD_TLS_SSL_DEBUG_MSG( 3, ( "client hello, total extension length: %d",
                   ext_len ) );
                                         ...
                                         size_t len )
{
    ...
    if( ssl->renego_status != JHD_TLS_SSL_INITIAL_HANDSHAKE )
    {
        ...
        if( len    != 1 + ssl->verify_data_len * 2 ||
            ...
            jhd_tls_ssl_safer_memcmp( buf + 1,
                          ssl->own_verify_data, ssl->verify_data_len ) != 0 ||
            jhd_tls_ssl_safer_memcmp( buf + 1 + ssl->verify_data_len,
                                         ...
                                         size_t len )
{
    if( ssl->conf->session_tickets == JHD_TLS_SSL_SESSION_TICKETS_DISABLED ||
        len != 0 )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "non-matching session ticket extension" ) );
        jhd_tls_ssl_send_alert_message( ssl, JHD_TLS_SSL_ALERT_LEVEL_FATAL,
                                        JHD_TLS_SSL_ALERT_MSG_HANDSHAKE_FAILURE );
        return( JHD_TLS_ERR_SSL_BAD_HS_SERVER_HELLO );
    }
...
#if defined(JHD_TLS_ECDH_C) || defined(JHD_TLS_ECDSA_C) || \
    defined(JHD_TLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
static int ssl_parse_supported_point_formats_ext( jhd_tls_ssl_context *ssl,
                                                  const unsigned char *buf,
                                                  size_t len )
{
    size_t list_size;
                                   ...
                                   size_t len )
{
    ...
    jhd_tls_free( ssl->handshake->ecjpake_cache );
 ...
 */
#if defined(JHD_TLS_SSL_PROTO_DTLS)
static int ssl_parse_hello_verify_request( jhd_tls_ssl_context *ssl )
{
    const unsigned char *p = ssl->in_msg + jhd_tls_ssl_hs_hdr_len( ssl );
    int major_ver, minor_ver;
    unsigned char cookie_len;

    JHD_TLS_SSL_DEBUG_MSG( 2, ( "=> parse hello verify request" ) );
    ...
    jhd_tls_ssl_read_version( &major_ver, &minor_ver, ssl->conf->transport, p );
    p += 2;
     ...
     */
    if( major_ver < JHD_TLS_SSL_MAJOR_VERSION_3 ||
        minor_ver < JHD_TLS_SSL_MINOR_VERSION_2 ||
        major_ver > ssl->conf->max_major_ver  ||
        minor_ver > ssl->conf->max_minor_ver  )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "bad server version" ) );
    ...
    cookie_len = *p++;
    JHD_TLS_SSL_DEBUG_BUF( 3, "cookie", p, cookie_len );
    ...
    jhd_tls_free( ssl->handshake->verify_cookie );

    ssl->handshake->verify_cookie = jhd_tls_calloc( 1, cookie_len );
    if( ssl->handshake->verify_cookie  == NULL )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "alloc failed (%d bytes)", cookie_len ) );
    ...
    jhd_tls_ssl_reset_checksum( ssl );
...
static int ssl_parse_server_hello( jhd_tls_ssl_context *ssl )
{
    ...
    if( ssl->in_msgtype != JHD_TLS_SSL_MSG_HANDSHAKE )
    {
        ...
        if( ssl->renego_status == JHD_TLS_SSL_RENEGOTIATION_IN_PROGRESS )
        {
            ...
            JHD_TLS_SSL_DEBUG_MSG( 1, ( "non-handshake message during renego" ) );

            ssl->keep_current_message = 1;
            return( JHD_TLS_ERR_SSL_WAITING_SERVER_HELLO_RENEGO );
        }
#endif /* JHD_TLS_SSL_RENEGOTIATION */
    ...
    if( ssl->conf->transport == JHD_TLS_SSL_TRANSPORT_DATAGRAM )
    {
        if( buf[0] == JHD_TLS_SSL_HS_HELLO_VERIFY_REQUEST )
        {
            JHD_TLS_SSL_DEBUG_MSG( 2, ( "received hello verify request" ) );
            JHD_TLS_SSL_DEBUG_MSG( 2, ( "<= parse server hello" ) );
    ...
    if( ssl->in_hslen < 38 + jhd_tls_ssl_hs_hdr_len( ssl ) ||
        buf[0] != JHD_TLS_SSL_HS_SERVER_HELLO )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "bad server hello message" ) );
        jhd_tls_ssl_send_alert_message( ssl, JHD_TLS_SSL_ALERT_LEVEL_FATAL,
                                        JHD_TLS_SSL_ALERT_MSG_DECODE_ERROR );
    ...
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "server version out of bounds - "
    ...
    JHD_TLS_SSL_DEBUG_MSG( 3, ( "server hello, current time: %lu",
                           ( (uint32_t) buf[2] << 24 ) |
                           ( (uint32_t) buf[3] << 16 ) |
                           ( (uint32_t) buf[4] <<  8 ) |
                           ( (uint32_t) buf[5]       ) ) );
    ...
    if( ssl->in_hslen > jhd_tls_ssl_hs_hdr_len( ssl ) + 39 + n )
    {
        ...
        if( ( ext_len > 0 && ext_len < 4 ) ||
            ssl->in_hslen != jhd_tls_ssl_hs_hdr_len( ssl ) + 40 + n + ext_len )
        {
            JHD_TLS_SSL_DEBUG_MSG( 1, ( "bad server hello message" ) );
            jhd_tls_ssl_send_alert_message( ssl, JHD_TLS_SSL_ALERT_LEVEL_FATAL,
                                            JHD_TLS_SSL_ALERT_MSG_DECODE_ERROR );
    ...
    if( comp != JHD_TLS_SSL_COMPRESS_NULL &&
        ( comp != JHD_TLS_SSL_COMPRESS_DEFLATE || accept_comp == 0 ) )
#else /* JHD_TLS_ZLIB_SUPPORT */
    if( comp != JHD_TLS_SSL_COMPRESS_NULL )
    ...
    if( comp != JHD_TLS_SSL_COMPRESS_NULL
#if defined(JHD_TLS_ZLIB_SUPPORT)
        && comp != JHD_TLS_SSL_COMPRESS_DEFLATE
#endif
      )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "bad server hello message" ) );
    ...
    while( ext_len )
    {
        ...
        case JHD_TLS_TLS_EXT_SESSION_TICKET:
            JHD_TLS_SSL_DEBUG_MSG( 3, ( "found session_ticket extension" ) );
    ...
    defined(JHD_TLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
        ...
        case JHD_TLS_TLS_EXT_ECJPAKE_KKPP:
            JHD_TLS_SSL_DEBUG_MSG( 3, ( "found ecjpake_kkpp extension" ) );
...
static int ssl_parse_server_dh_params( jhd_tls_ssl_context *ssl, unsigned char **p,
                                       unsigned char *end )
{
     ...
     * } ServerDHParams;
     */
    if( ( ret = jhd_tls_dhm_read_params( &ssl->handshake->dhm_ctx, p, end ) ) != 0 )
    {
        JHD_TLS_SSL_DEBUG_RET( 2, ( "jhd_tls_dhm_read_params" ), ret );
        return( ret );
    }

    if( ssl->handshake->dhm_ctx.len * 8 < ssl->conf->dhm_min_bitlen )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "DHM prime too short: %d < %d",
...
static int ssl_check_server_ecdh_params( const jhd_tls_ssl_context *ssl )
{
    ...
    defined(JHD_TLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED) ||                   \
    defined(JHD_TLS_KEY_EXCHANGE_ECDHE_PSK_ENABLED)
static int ssl_parse_server_ecdh_params( jhd_tls_ssl_context *ssl,
                                         unsigned char **p,
                                         unsigned char *end )
{
    int ret = JHD_TLS_ERR_SSL_FEATURE_UNAVAILABLE;
     ...
     * } ServerECDHParams;
     */
    if( ( ret = jhd_tls_ecdh_read_params( &ssl->handshake->ecdh_ctx,
...
static int ssl_parse_server_psk_hint( jhd_tls_ssl_context *ssl,
                                      unsigned char **p,
                                      unsigned char *end )
{
    int ret = JHD_TLS_ERR_SSL_FEATURE_UNAVAILABLE;
 ...
 */
static int ssl_write_encrypted_pms( jhd_tls_ssl_context *ssl,
                                    ...
                                    size_t pms_offset )
{
    ...
    {
        JHD_TLS_SSL_DEBUG_RET( 1, "f_rng", ret );
        return( ret );
    }
    ...
    if( ( ret = jhd_tls_pk_encrypt( &ssl->session_negotiate->peer_cert->pk,
                            p, ssl->handshake->pmslen,
                            ssl->out_msg + offset + len_bytes, olen,
                            JHD_TLS_SSL_MAX_CONTENT_LEN - offset - len_bytes,
    ...
    defined(JHD_TLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED)
static int ssl_parse_signature_algorithm( jhd_tls_ssl_context *ssl,
                                          unsigned char **p,
                                          unsigned char *end,
                                          ...
                                          jhd_tls_pk_type_t *pk_alg )
{
    ...
    *pk_alg = JHD_TLS_PK_NONE;
    ...
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "Server used unsupported "
                            "HashAlgorithm %d", *(p)[0] ) );
        return( JHD_TLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE );
    }
    ...
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "server used HashAlgorithm %d that was not offered",
                                    *(p)[0] ) );
        return( JHD_TLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE );
    }
...
{
    int ret;
    const jhd_tls_ecp_keypair *peer_key;
    ...
    peer_key = jhd_tls_pk_ec( ssl->session_negotiate->peer_cert->pk );

    if( ( ret = jhd_tls_ecdh_get_params( &ssl->handshake->ecdh_ctx, peer_key,
                                 JHD_TLS_ECDH_THEIRS ) ) != 0 )
    {
    ...
    if( ssl_check_server_ecdh_params( ssl ) != 0 )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "bad server certificate (ECDH curve)" ) );
...
static int ssl_parse_server_key_exchange( jhd_tls_ssl_context *ssl )
{
...
#if defined(JHD_TLS_KEY_EXCHANGE_ECDH_RSA_ENABLED) || \
    defined(JHD_TLS_KEY_EXCHANGE_ECDH_ECDSA_ENABLED)
    if( ciphersuite_info->key_exchange == JHD_TLS_KEY_EXCHANGE_ECDH_RSA ||
        ciphersuite_info->key_exchange == JHD_TLS_KEY_EXCHANGE_ECDH_ECDSA )
    {
        if( ( ret = ssl_get_ecdh_params_from_cert( ssl ) ) != 0 )
        {
            JHD_TLS_SSL_DEBUG_RET( 1, "ssl_get_ecdh_params_from_cert", ret );
            jhd_tls_ssl_send_alert_message( ssl, JHD_TLS_SSL_ALERT_LEVEL_FATAL,
                                            JHD_TLS_SSL_ALERT_MSG_HANDSHAKE_FAILURE );
            return( ret );
        }
    ...
    if( ssl->in_msg[0] != JHD_TLS_SSL_HS_SERVER_KEY_EXCHANGE )
    {
        ...
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "server key exchange message must "
                                    "not be skipped" ) );
        jhd_tls_ssl_send_alert_message( ssl, JHD_TLS_SSL_ALERT_LEVEL_FATAL,
                                        JHD_TLS_SSL_ALERT_MSG_UNEXPECTED_MESSAGE );
    ...
    defined(JHD_TLS_SSL_PROTO_TLS1_1)
        ...
        {
            hashlen = 36;
            ret = jhd_tls_ssl_get_key_exchange_md_ssl_tls( ssl, hash, params,
                                                           params_len );
            if( ret != 0 )
                return( ret );
        }
...
static int ssl_parse_certificate_request( jhd_tls_ssl_context *ssl )
{
    ...
    ssl->state++;
    ssl->client_auth = ( ssl->in_msg[0] == JHD_TLS_SSL_HS_CERTIFICATE_REQUEST );

    JHD_TLS_SSL_DEBUG_MSG( 3, ( "got %s certificate request",
    ...
    if( ssl->minor_ver == JHD_TLS_SSL_MINOR_VERSION_3 )
    {
        size_t sig_alg_len = ( ( buf[jhd_tls_ssl_hs_hdr_len( ssl ) + 1 + n] <<  8 )
                             | ( buf[jhd_tls_ssl_hs_hdr_len( ssl ) + 2 + n]       ) );
        ...
        {
            JHD_TLS_SSL_DEBUG_MSG( 3, ( "Supported Signature Algorithm found: %d"
    ...
    dn_len = ( ( buf[jhd_tls_ssl_hs_hdr_len( ssl ) + 1 + n] <<  8 )
             | ( buf[jhd_tls_ssl_hs_hdr_len( ssl ) + 2 + n]       ) );
...
static int ssl_parse_server_hello_done( jhd_tls_ssl_context *ssl )
{
    ...
    if( ssl->in_hslen  != jhd_tls_ssl_hs_hdr_len( ssl ) ||
        ssl->in_msg[0] != JHD_TLS_SSL_HS_SERVER_HELLO_DONE )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "bad server hello done message" ) );
        jhd_tls_ssl_send_alert_message( ssl, JHD_TLS_SSL_ALERT_LEVEL_FATAL,
                                        JHD_TLS_SSL_ALERT_MSG_DECODE_ERROR );
...
static int ssl_write_client_key_exchange( jhd_tls_ssl_context *ssl )
{
    ...
    if( ciphersuite_info->key_exchange == JHD_TLS_KEY_EXCHANGE_DHE_RSA )
    {
        ...
        ret = jhd_tls_dhm_make_public( &ssl->handshake->dhm_ctx,
                                (int) jhd_tls_mpi_size( &ssl->handshake->dhm_ctx.P ),
                               &ssl->out_msg[i], n,
                                ssl->conf->f_rng, ssl->conf->p_rng );
        if( ret != 0 )
        {
            JHD_TLS_SSL_DEBUG_RET( 1, "jhd_tls_dhm_make_public", ret );
            return( ret );
        }
        ...
        ciphersuite_info->key_exchange == JHD_TLS_KEY_EXCHANGE_ECDH_ECDSA )
    {
        ...
        ret = jhd_tls_ecdh_make_public( &ssl->handshake->ecdh_ctx,
                                &n,
                                &ssl->out_msg[i], 1000,
                                ssl->conf->f_rng, ssl->conf->p_rng );
        if( ret != 0 )
        ...
        JHD_TLS_SSL_DEBUG_MPI( 3, "ECDH: z", &ssl->handshake->ecdh_ctx.z );
          ...
          JHD_TLS_KEY_EXCHANGE_ECDH_ECDSA_ENABLED */
#if defined(JHD_TLS_KEY_EXCHANGE__SOME__PSK_ENABLED)
    if( jhd_tls_ssl_ciphersuite_uses_psk( ciphersuite_info ) )
    {
        /*
        ...
        {
            JHD_TLS_SSL_DEBUG_MSG( 1, ( "got no private key for PSK" ) );
        ...
        if( i + 2 + n > JHD_TLS_SSL_MAX_CONTENT_LEN )
        {
            JHD_TLS_SSL_DEBUG_MSG( 1, ( "psk identity too long or "
        ...
        if( ciphersuite_info->key_exchange == JHD_TLS_KEY_EXCHANGE_DHE_PSK )
        {
            ...
            if( i + 2 + n > JHD_TLS_SSL_MAX_CONTENT_LEN )
            {
                JHD_TLS_SSL_DEBUG_MSG( 1, ( "psk identity or DHM size too long"
        ...
        if( ciphersuite_info->key_exchange == JHD_TLS_KEY_EXCHANGE_ECDHE_PSK )
        {
             ...
             */
            ret = jhd_tls_ecdh_make_public( &ssl->handshake->ecdh_ctx, &n,
                    &ssl->out_msg[i], JHD_TLS_SSL_MAX_CONTENT_LEN - i,
                    ssl->conf->f_rng, ssl->conf->p_rng );
            if( ret != 0 )
            {
                JHD_TLS_SSL_DEBUG_RET( 1, "jhd_tls_ecdh_make_public", ret );
    ...
    if( ciphersuite_info->key_exchange == JHD_TLS_KEY_EXCHANGE_ECJPAKE )
    {
        ...
        ret = jhd_tls_ecjpake_write_round_two( &ssl->handshake->ecjpake_ctx,
                ssl->out_msg + i, JHD_TLS_SSL_MAX_CONTENT_LEN - i, &n,
                ssl->conf->f_rng, ssl->conf->p_rng );
        if( ret != 0 )
        {
...
static int ssl_write_certificate_verify( jhd_tls_ssl_context *ssl )
{
    ...
    if( ( ret = jhd_tls_pk_sign( jhd_tls_ssl_own_key( ssl ), md_alg, hash_start, hashlen,
...
#if defined(JHD_TLS_SSL_SESSION_TICKETS)
static int ssl_parse_new_session_ticket( jhd_tls_ssl_context *ssl )
{
    int ret;
    uint32_t lifetime;
    size_t ticket_len;
    ...
    JHD_TLS_SSL_DEBUG_MSG( 2, ( "=> parse new session ticket" ) );
    ...
    if( ssl->in_msgtype != JHD_TLS_SSL_MSG_HANDSHAKE )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "bad new session ticket message" ) );
        jhd_tls_ssl_send_alert_message( ssl, JHD_TLS_SSL_ALERT_LEVEL_FATAL,
                                        JHD_TLS_SSL_ALERT_MSG_UNEXPECTED_MESSAGE );
        return( JHD_TLS_ERR_SSL_UNEXPECTED_MESSAGE );
    }
     ...
     */
    if( ssl->in_msg[0] != JHD_TLS_SSL_HS_NEW_SESSION_TICKET ||
        ssl->in_hslen < 6 + jhd_tls_ssl_hs_hdr_len( ssl ) )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "bad new session ticket message" ) );
        jhd_tls_ssl_send_alert_message( ssl, JHD_TLS_SSL_ALERT_LEVEL_FATAL,
                                        JHD_TLS_SSL_ALERT_MSG_DECODE_ERROR );
        return( JHD_TLS_ERR_SSL_BAD_HS_NEW_SESSION_TICKET );
    }

    msg = ssl->in_msg + jhd_tls_ssl_hs_hdr_len( ssl );
    ...
    if( ticket_len + 6 + jhd_tls_ssl_hs_hdr_len( ssl ) != ssl->in_hslen )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "bad new session ticket message" ) );
        jhd_tls_ssl_send_alert_message( ssl, JHD_TLS_SSL_ALERT_LEVEL_FATAL,
                                        JHD_TLS_SSL_ALERT_MSG_DECODE_ERROR );
    ...
    JHD_TLS_SSL_DEBUG_MSG( 3, ( "ticket length: %d", ticket_len ) );
    ...
    jhd_tls_platform_zeroize( ssl->session_negotiate->ticket,
                              ssl->session_negotiate->ticket_len );
    jhd_tls_free( ssl->session_negotiate->ticket );
    ssl->session_negotiate->ticket = NULL;
    ssl->session_negotiate->ticket_len = 0;

    if( ( ticket = jhd_tls_calloc( 1, ticket_len ) ) == NULL )
    {
        JHD_TLS_SSL_DEBUG_MSG( 1, ( "ticket alloc failed" ) );
        jhd_tls_ssl_send_alert_message( ssl, JHD_TLS_SSL_ALERT_LEVEL_FATAL,
                                        JHD_TLS_SSL_ALERT_MSG_INTERNAL_ERROR );
        return( JHD_TLS_ERR_SSL_ALLOC_FAILED );
    }
     ...
     */
    JHD_TLS_SSL_DEBUG_MSG( 3, ( "ticket in use, discarding session id" ) );
...
int jhd_tls_ssl_handshake_client_step( jhd_tls_ssl_context *ssl )
{
    ...
    /* Change state now, so that it is right in jhd_tls_ssl_read_record(), used
     * by DTLS for dropping out-of-sequence ChangeCipherSpec records */
#if defined(JHD_TLS_SSL_SESSION_TICKETS)
    if( ssl->state == JHD_TLS_SSL_SERVER_CHANGE_CIPHER_SPEC &&
        ssl->handshake->new_session_ticket != 0 )
    {
        ssl->state = JHD_TLS_SSL_SERVER_NEW_SESSION_TICKET;
    }
#endif
    ...
    switch( ssl->state )
    {
        ...
        */
#if defined(JHD_TLS_SSL_SESSION_TICKETS)
       case JHD_TLS_SSL_SERVER_NEW_SESSION_TICKET:
