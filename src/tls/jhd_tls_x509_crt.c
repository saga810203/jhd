...
const jhd_tls_x509_crt_profile jhd_tls_x509_crt_profile_default =
{
    ...
    JHD_TLS_X509_ID_FLAG( JHD_TLS_MD_SHA1 ) |
#endif
    /* Only SHA-2 hashes */
    JHD_TLS_X509_ID_FLAG( JHD_TLS_MD_SHA224 ) |
    JHD_TLS_X509_ID_FLAG( JHD_TLS_MD_SHA256 ) |
    JHD_TLS_X509_ID_FLAG( JHD_TLS_MD_SHA384 ) |
    JHD_TLS_X509_ID_FLAG( JHD_TLS_MD_SHA512 ),
...
const jhd_tls_x509_crt_profile jhd_tls_x509_crt_profile_next =
{
    ...
    JHD_TLS_X509_ID_FLAG( JHD_TLS_ECP_DP_SECP256K1 ),
#else
    0,
#endif
    2048,
...
const jhd_tls_x509_crt_profile jhd_tls_x509_crt_profile_suiteb =
{
    ...
    JHD_TLS_X509_ID_FLAG( JHD_TLS_MD_SHA384 ),
    /* Only ECDSA */
    JHD_TLS_X509_ID_FLAG( JHD_TLS_PK_ECDSA ) |
    JHD_TLS_X509_ID_FLAG( JHD_TLS_PK_ECKEY ),
#if defined(JHD_TLS_ECP_C)
 ...
 */
static int x509_profile_check_md_alg( const jhd_tls_x509_crt_profile *profile,
                                      jhd_tls_md_type_t md_alg )
{
    if( ( profile->allowed_mds & JHD_TLS_X509_ID_FLAG( md_alg ) ) != 0 )
        return( 0 );
 ...
 */
static int x509_profile_check_pk_alg( const jhd_tls_x509_crt_profile *profile,
                                      jhd_tls_pk_type_t pk_alg )
{
    if( ( profile->allowed_pks & JHD_TLS_X509_ID_FLAG( pk_alg ) ) != 0 )
        return( 0 );
 ...
 */
static int x509_profile_check_key( const jhd_tls_x509_crt_profile *profile,
                                   const jhd_tls_pk_context *pk )
{
    const jhd_tls_pk_type_t pk_alg = jhd_tls_pk_get_type( pk );

#if defined(JHD_TLS_RSA_C)
    if( pk_alg == JHD_TLS_PK_RSA || pk_alg == JHD_TLS_PK_RSASSA_PSS )
    {
        if( jhd_tls_pk_get_bitlen( pk ) >= profile->rsa_min_bitlen )
            return( 0 );
    ...
    if( pk_alg == JHD_TLS_PK_ECDSA ||
        pk_alg == JHD_TLS_PK_ECKEY ||
        pk_alg == JHD_TLS_PK_ECKEY_DH )
    {
        const jhd_tls_ecp_group_id gid = jhd_tls_pk_ec( *pk )->grp.id;

        if( ( profile->allowed_curves & JHD_TLS_X509_ID_FLAG( gid ) ) != 0 )
            return( 0 );
                             ...
                             int *ver )
{
    ...
    if( *p != end )
        return( JHD_TLS_ERR_X509_INVALID_VERSION +
...
static int x509_get_dates( unsigned char **p,
                           const unsigned char *end,
                           jhd_tls_x509_time *from,
                           jhd_tls_x509_time *to )
{
    int ret;
    size_t len;
                         ...
                         jhd_tls_x509_buf *uid, int n )
{
    ...
    if( ( ret = jhd_tls_asn1_get_tag( p, end, &uid->len,
            JHD_TLS_ASN1_CONTEXT_SPECIFIC | JHD_TLS_ASN1_CONSTRUCTED | n ) ) != 0 )
    {
        if( ret == JHD_TLS_ERR_ASN1_UNEXPECTED_TAG )
            return( 0 );
                                       ...
                                       unsigned char *ns_cert_type)
{
    ...
    if( bs.len != 1 )
        return( JHD_TLS_ERR_X509_INVALID_EXTENSIONS +
                JHD_TLS_ERR_ASN1_INVALID_LENGTH );
...
static int x509_get_ext_key_usage( unsigned char **p,
                               const unsigned char *end,
                               jhd_tls_x509_sequence *ext_key_usage)
{
    int ret;
...
static int x509_get_subject_alt_name( unsigned char **p,
                                      const unsigned char *end,
                                      jhd_tls_x509_sequence *subject_alt_name )
{
    int ret;
    size_t len, tag_len;
    ...
    while( *p < end )
    {
        ...
        if( ( tag & JHD_TLS_ASN1_TAG_CLASS_MASK ) !=
        ...
        if( cur->buf.p != NULL )
        {
            ...
            cur->next = jhd_tls_calloc( 1, sizeof( jhd_tls_asn1_sequence ) );

            if( cur->next == NULL )
                return( JHD_TLS_ERR_X509_INVALID_EXTENSIONS +
                        JHD_TLS_ERR_ASN1_ALLOC_FAILED );
                             ...
                             jhd_tls_x509_crt *crt )
{
    ...
    while( *p < end )
    {
        ...
        *p += extn_oid.len;
        ...
        if( ( ret = jhd_tls_asn1_get_bool( p, end_ext_data, &is_critical ) ) != 0 &&
         ...
         */
        ret = jhd_tls_oid_get_x509_ext_type( &extn_oid, &ext_type );
 ...
 */
static int x509_crt_parse_der_core( jhd_tls_x509_crt *crt, const unsigned char *buf,
                                    size_t buflen )
{
    int ret;
    size_t len;
    ...
    crt->raw.p = p = jhd_tls_calloc( 1, crt->raw.len );
    if( p == NULL )
    ...
    if( ( ret = x509_get_version(  &p, end, &crt->version  ) ) != 0 ||
        ( ret = jhd_tls_x509_get_serial(   &p, end, &crt->serial   ) ) != 0 ||
        ( ret = jhd_tls_x509_get_alg(      &p, end, &crt->sig_oid,
                                            &sig_params1 ) ) != 0 )
    {
        jhd_tls_x509_crt_free( crt );
        return( ret );
    ...
    if( ( ret = jhd_tls_x509_get_sig_alg( &crt->sig_oid, &sig_params1,
    ...
    if( len && ( ret = jhd_tls_x509_get_name( &p, p + len, &crt->subject ) ) != 0 )
    {
        jhd_tls_x509_crt_free( crt );
        return( ret );
    }
     ...
     */
    if( ( ret = jhd_tls_pk_parse_subpubkey( &p, end, &crt->pk ) ) != 0 )
    {
        jhd_tls_x509_crt_free( crt );
        return( ret );
    }
 ...
 * chained list
 */
int jhd_tls_x509_crt_parse_der( jhd_tls_x509_crt *chain, const unsigned char *buf,
                        size_t buflen )
{
    int ret;
    jhd_tls_x509_crt *crt = chain, *prev = NULL;
    ...
    {
        crt->next = jhd_tls_calloc( 1, sizeof( jhd_tls_x509_crt ) );

        if( crt->next == NULL )
            return( JHD_TLS_ERR_X509_ALLOC_FAILED );

        prev = crt;
        jhd_tls_x509_crt_init( crt->next );
        crt = crt->next;
    }
 ...
 * list
 */
int jhd_tls_x509_crt_parse( jhd_tls_x509_crt *chain, const unsigned char *buf, size_t buflen )
{
#if defined(JHD_TLS_PEM_PARSE_C)
    int success = 0, first_error = 0, total_failed = 0;
    int buf_format = JHD_TLS_X509_FORMAT_DER;
#endif
    ...
    {
        buf_format = JHD_TLS_X509_FORMAT_PEM;
    }

    if( buf_format == JHD_TLS_X509_FORMAT_DER )
        return jhd_tls_x509_crt_parse_der( chain, buf, buflen );
#else
    return jhd_tls_x509_crt_parse_der( chain, buf, buflen );
#endif

#if defined(JHD_TLS_PEM_PARSE_C)
    if( buf_format == JHD_TLS_X509_FORMAT_PEM )
    {
        int ret;
        ...
        while( buflen > 1 )
        {
            ...
            else if( ret == JHD_TLS_ERR_PEM_BAD_INPUT_DATA )
            {
                return( ret );
            }
            ...
            ret = jhd_tls_x509_crt_parse_der( chain, pem.buf, pem.buflen );
            ...
            if( ret != 0 )
            {
                 ...
                 */
                if( ret == JHD_TLS_ERR_X509_ALLOC_FAILED )
                    return( ret );
    ...
    if( success )
        ...
        return( JHD_TLS_ERR_X509_CERT_UNKNOWN_FORMAT );
 ...
 */
int jhd_tls_x509_crt_parse_file( jhd_tls_x509_crt *chain, const char *path )
{
    int ret;
    size_t n;
    unsigned char *buf;
    ...
    ret = jhd_tls_x509_crt_parse( chain, buf, n );
...
int jhd_tls_x509_crt_parse_path( jhd_tls_x509_crt *chain, const char *path )
{
    int ret = 0;
#if defined(_WIN32) && !defined(EFIX64) && !defined(EFI32)
    int w_ret;
    ...
    if( hFind == INVALID_HANDLE_VALUE )
        return( JHD_TLS_ERR_X509_FILE_IO_ERROR );
    ...
    do
    {
        ...
        if( w_ret == 0 )
        {
            ret = JHD_TLS_ERR_X509_FILE_IO_ERROR;
            goto cleanup;
        }

        w_ret = jhd_tls_x509_crt_parse_file( chain, filename );
        if( w_ret < 0 )
            ret++;
        else
            ret += w_ret;
...
cleanup:
    ...
    struct stat sb;
    struct dirent *entry;
    char entry_name[JHD_TLS_X509_MAX_FILE_PATH_LEN];
...
#if defined(JHD_TLS_THREADING_C)
    if( ( ret = jhd_tls_mutex_lock( &jhd_tls_threading_readdir_mutex ) ) != 0 )
    {
        closedir( dir );
        return( ret );
    }
    ...
    {
        snp_ret = jhd_tls_snprintf( entry_name, sizeof entry_name,
                                    "%s/%s", path, entry->d_name );

        if( snp_ret < 0 || (size_t)snp_ret >= sizeof entry_name )
        {
            ret = JHD_TLS_ERR_X509_BUFFER_TOO_SMALL;
            goto cleanup;
        }
        ...
        // Ignore parse errors
        //
        t_ret = jhd_tls_x509_crt_parse_file( chain, entry_name );
        if( t_ret < 0 )
            ret++;
        else
            ret += t_ret;
...
#if defined(JHD_TLS_THREADING_C)
    if( jhd_tls_mutex_unlock( &jhd_tls_threading_readdir_mutex ) != 0 )
        ret = JHD_TLS_ERR_THREADING_MUTEX_ERROR;
#endif /* JHD_TLS_THREADING_C */
...
static int x509_info_subject_alt_name( char **buf, size_t *size,
                                       const jhd_tls_x509_sequence *subject_alt_name )
{
    size_t i;
    size_t n = *size;
    char *p = *buf;
...
#define PRINT_ITEM(i)                           \
    {                                           \
        ret = jhd_tls_snprintf( p, n, "%s" i, sep );    \
...
static int x509_info_ext_key_usage( char **buf, size_t *size,
                                    const jhd_tls_x509_sequence *extended_key_usage )
{
    int ret;
    const char *desc;
    size_t n = *size;
    ...
    while( cur != NULL )
    {
        if( jhd_tls_oid_get_extended_key_usage( &cur->buf, &desc ) != 0 )
            desc = "???";

        ret = jhd_tls_snprintf( p, n, "%s%s", sep, desc );
        JHD_TLS_X509_SAFE_SNPRINTF;
...
#define BC              "18"
int jhd_tls_x509_crt_info( char *buf, size_t size, const char *prefix,
                   const jhd_tls_x509_crt *crt )
{
    ...
    ret = jhd_tls_snprintf( p, n, "%scert. version     : %d\n",
                               prefix, crt->version );
    JHD_TLS_X509_SAFE_SNPRINTF;
    ret = jhd_tls_snprintf( p, n, "%sserial number     : ",
                               prefix );
    JHD_TLS_X509_SAFE_SNPRINTF;
    ...
    ret = jhd_tls_x509_sig_alg_gets( p, n, &crt->sig_oid, crt->sig_pk,
                             crt->sig_md, crt->sig_opts );
    JHD_TLS_X509_SAFE_SNPRINTF;

    /* Key size */
    if( ( ret = jhd_tls_x509_key_size_helper( key_size_str, BEFORE_COLON,
                                      jhd_tls_pk_get_name( &crt->pk ) ) ) != 0 )
    {
        return( ret );
    }

    ret = jhd_tls_snprintf( p, n, "\n%s%-" BC "s: %d bits", prefix, key_size_str,
                          (int) jhd_tls_pk_get_bitlen( &crt->pk ) );
    JHD_TLS_X509_SAFE_SNPRINTF;
    ...
    if( crt->ext_types & JHD_TLS_X509_EXT_BASIC_CONSTRAINTS )
    {
        ...
        {
            ret = jhd_tls_snprintf( p, n, ", max_pathlen=%d", crt->max_pathlen - 1 );
            JHD_TLS_X509_SAFE_SNPRINTF;
        }
    }
...
static const struct x509_crt_verify_string x509_crt_verify_strings[] = {
    { JHD_TLS_X509_BADCERT_EXPIRED,       "The certificate validity has expired" },
    { JHD_TLS_X509_BADCERT_REVOKED,       "The certificate has been revoked (is on a CRL)" },
    { JHD_TLS_X509_BADCERT_CN_MISMATCH,   "The certificate Common Name (CN) does not match with the expected CN" },
    { JHD_TLS_X509_BADCERT_NOT_TRUSTED,   "The certificate is not correctly signed by the trusted CA" },
    { JHD_TLS_X509_BADCRL_NOT_TRUSTED,    "The CRL is not correctly signed by the trusted CA" },
    { JHD_TLS_X509_BADCRL_EXPIRED,        "The CRL is expired" },
    { JHD_TLS_X509_BADCERT_MISSING,       "Certificate was missing" },
    { JHD_TLS_X509_BADCERT_SKIP_VERIFY,   "Certificate verification was skipped" },
    { JHD_TLS_X509_BADCERT_OTHER,         "Other reason (can be used by verify callback)" },
    { JHD_TLS_X509_BADCERT_FUTURE,        "The certificate validity starts in the future" },
    { JHD_TLS_X509_BADCRL_FUTURE,         "The CRL is from the future" },
    { JHD_TLS_X509_BADCERT_KEY_USAGE,     "Usage does not match the keyUsage extension" },
    { JHD_TLS_X509_BADCERT_EXT_KEY_USAGE, "Usage does not match the extendedKeyUsage extension" },
    { JHD_TLS_X509_BADCERT_NS_CERT_TYPE,  "Usage does not match the nsCertType extension" },
    { JHD_TLS_X509_BADCERT_BAD_MD,        "The certificate is signed with an unacceptable hash." },
    { JHD_TLS_X509_BADCERT_BAD_PK,        "The certificate is signed with an unacceptable PK alg (eg RSA vs ECDSA)." },
    { JHD_TLS_X509_BADCERT_BAD_KEY,       "The certificate is signed with an unacceptable key (eg bad curve, RSA too short)." },
    { JHD_TLS_X509_BADCRL_BAD_MD,         "The CRL is signed with an unacceptable hash." },
    { JHD_TLS_X509_BADCRL_BAD_PK,         "The CRL is signed with an unacceptable PK alg (eg RSA vs ECDSA)." },
    { JHD_TLS_X509_BADCRL_BAD_KEY,        "The CRL is signed with an unacceptable key (eg bad curve, RSA too short)." },
    { 0, NULL }
};

int jhd_tls_x509_crt_verify_info( char *buf, size_t size, const char *prefix,
                          uint32_t flags )
{
    int ret;
    ...
    if( flags != 0 )
    {
        ret = jhd_tls_snprintf( p, n, "%sUnknown reason "
...
int jhd_tls_x509_crt_check_key_usage( const jhd_tls_x509_crt *crt,
                                      unsigned int usage )
{
    unsigned int usage_must, usage_may;
    unsigned int may_mask = JHD_TLS_X509_KU_ENCIPHER_ONLY
                          | JHD_TLS_X509_KU_DECIPHER_ONLY;
...
int jhd_tls_x509_crt_check_extended_key_usage( const jhd_tls_x509_crt *crt,
 ...
 */
int jhd_tls_x509_crt_is_revoked( const jhd_tls_x509_crt *crt, const jhd_tls_x509_crl *crl )
{
    const jhd_tls_x509_crl_entry *cur = &crl->entry;
    ...
    while( cur != NULL && cur->serial.len != 0 )
    {
        ...
        {
            if( jhd_tls_x509_time_is_past( &cur->revocation_date ) )
                return( 1 );
        }
 ...
 */
static int x509_crt_verifycrl( jhd_tls_x509_crt *crt, jhd_tls_x509_crt *ca,
                               jhd_tls_x509_crl *crl_list,
                               const jhd_tls_x509_crt_profile *profile )
{
    ...
    while( crl_list != NULL )
    {
         ...
         */
#if defined(JHD_TLS_X509_CHECK_KEY_USAGE)
        if( jhd_tls_x509_crt_check_key_usage( ca, JHD_TLS_X509_KU_CRL_SIGN ) != 0 )
        {
            flags |= JHD_TLS_X509_BADCRL_NOT_TRUSTED;
            break;
        }
#endif
        ...
        if( x509_profile_check_md_alg( profile, crl_list->sig_md ) != 0 )
            flags |= JHD_TLS_X509_BADCRL_BAD_MD;

        if( x509_profile_check_pk_alg( profile, crl_list->sig_pk ) != 0 )
            flags |= JHD_TLS_X509_BADCRL_BAD_PK;

        md_info = jhd_tls_md_info_from_type( crl_list->sig_md );
        if( jhd_tls_md( md_info, crl_list->tbs.p, crl_list->tbs.len, hash ) != 0 )
        {
        ...
        if( jhd_tls_pk_verify_ext( crl_list->sig_pk, crl_list->sig_opts, &ca->pk,
                           crl_list->sig_md, hash, jhd_tls_md_get_size( md_info ),
         ...
         */
        if( jhd_tls_x509_time_is_past( &crl_list->next_update ) )
            flags |= JHD_TLS_X509_BADCRL_EXPIRED;

        if( jhd_tls_x509_time_is_future( &crl_list->this_update ) )
            flags |= JHD_TLS_X509_BADCRL_FUTURE;

        /*
         * Check if certificate is revoked
         */
        if( jhd_tls_x509_crt_is_revoked( crt, crl_list ) )
        {
            flags |= JHD_TLS_X509_BADCERT_REVOKED;
            break;
        }
 ...
 */
static int x509_check_wildcard( const char *cn, const jhd_tls_x509_buf *name )
{
    size_t i;
...
static int x509_string_cmp( const jhd_tls_x509_buf *a, const jhd_tls_x509_buf *b )
{
    ...
    if( ( a->tag == JHD_TLS_ASN1_UTF8_STRING || a->tag == JHD_TLS_ASN1_PRINTABLE_STRING ) &&
        ( b->tag == JHD_TLS_ASN1_UTF8_STRING || b->tag == JHD_TLS_ASN1_PRINTABLE_STRING ) &&
        a->len == b->len &&
 ...
 */
static int x509_crt_check_signature( const jhd_tls_x509_crt *child,
                                     jhd_tls_x509_crt *parent )
{
    const jhd_tls_md_info_t *md_info;
    unsigned char hash[JHD_TLS_MD_MAX_SIZE];

    md_info = jhd_tls_md_info_from_type( child->sig_md );
    if( jhd_tls_md( md_info, child->tbs.p, child->tbs.len, hash ) != 0 )
    {
        /* Note: this can't happen except after an internal error */
        return( -1 );
    }

    if( jhd_tls_pk_verify_ext( child->sig_pk, child->sig_opts, &parent->pk,
                child->sig_md, hash, jhd_tls_md_get_size( md_info ),
 ...
 */
static int x509_crt_check_parent( const jhd_tls_x509_crt *child,
                                  const jhd_tls_x509_crt *parent,
                                  int top )
{
    int need_ca_bit;
...
#if defined(JHD_TLS_X509_CHECK_KEY_USAGE)
    if( need_ca_bit &&
        jhd_tls_x509_crt_check_key_usage( parent, JHD_TLS_X509_KU_KEY_CERT_SIGN ) != 0 )
    {
        return( -1 );
    }
#endif
 ...
 */
static jhd_tls_x509_crt *x509_crt_find_parent_in( jhd_tls_x509_crt *child,
                                                  jhd_tls_x509_crt *candidates,
                                                  int top,
                                                  size_t path_cnt,
                                                  size_t self_cnt )
{
    ...
    for( parent = candidates; parent != NULL; parent = parent->next )
    {
        ...
        /* optional time check */
        if( jhd_tls_x509_time_is_past( &parent->valid_to ) ||
            jhd_tls_x509_time_is_future( &parent->valid_from ) )
        {
 ...
 *  - NULL if no suitable parent was found
 */
static jhd_tls_x509_crt *x509_crt_find_parent( jhd_tls_x509_crt *child,
                                               jhd_tls_x509_crt *trust_ca,
    ...
    *chain_len = 0;
    ...
    while( 1 ) {
        ...
        if( ! parent_is_trusted &&
            *chain_len > JHD_TLS_X509_MAX_INTERMEDIATE_CA )
        {
 ...
 * Check for CN match
 */
static int x509_crt_check_cn( const jhd_tls_x509_buf *name,
 ...
 */
static void x509_crt_verify_name( const jhd_tls_x509_crt *crt,
                                  const char *cn,
                                  uint32_t *flags )
{
    ...
    if( crt->ext_types & JHD_TLS_X509_EXT_SUBJECT_ALT_NAME )
    {
        ...
        if( cur == NULL )
            *flags |= JHD_TLS_X509_BADCERT_CN_MISMATCH;
    }
    else
    {
        for( name = &crt->subject; name != NULL; name = name->next )
        {
            if( JHD_TLS_OID_CMP( JHD_TLS_OID_AT_CN, &name->oid ) == 0 &&
 ...
 */
int jhd_tls_x509_crt_verify( jhd_tls_x509_crt *crt,
                     jhd_tls_x509_crt *trust_ca,
                     jhd_tls_x509_crl *ca_crl,
                     ...
                     void *p_vrfy )
{
    return( jhd_tls_x509_crt_verify_with_profile( crt, trust_ca, ca_crl,
                &jhd_tls_x509_crt_profile_default, cn, flags, f_vrfy, p_vrfy ) );
}
 ...
 */
int jhd_tls_x509_crt_verify_with_profile( jhd_tls_x509_crt *crt,
                     jhd_tls_x509_crt *trust_ca,
                     jhd_tls_x509_crl *ca_crl,
                     const jhd_tls_x509_crt_profile *profile,
                     const char *cn, uint32_t *flags,
                     ...
                     void *p_vrfy )
{
    ...
    *flags = 0;
    ...
    if( profile == NULL )
    {
        ret = JHD_TLS_ERR_X509_BAD_INPUT_DATA;
        goto exit;
    }
    ...
    pk_type = jhd_tls_pk_get_type( &crt->pk );
...
{
    memset( crt, 0, sizeof(jhd_tls_x509_crt) );
}
...
void jhd_tls_x509_crt_free( jhd_tls_x509_crt *crt )
{
    ...
    do
    {
        jhd_tls_pk_free( &cert_cur->pk );

#if defined(JHD_TLS_X509_RSASSA_PSS_SUPPORT)
        jhd_tls_free( cert_cur->sig_opts );
#endif
        ...
        while( seq_cur != NULL )
        {
            ...
            jhd_tls_platform_zeroize( seq_prv,
                                      sizeof( jhd_tls_x509_sequence ) );
            jhd_tls_free( seq_prv );
        }
        ...
        {
            jhd_tls_platform_zeroize( cert_cur->raw.p, cert_cur->raw.len );
            jhd_tls_free( cert_cur->raw.p );
        }
