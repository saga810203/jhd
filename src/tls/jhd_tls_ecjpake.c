/*
 *  Elliptic curve J-PAKE
 *
 *  Copyright (C) 2006-2015, ARM Limited, All Rights Reserved
 *  SPDX-License-Identifier: Apache-2.0
 *
 *  Licensed under the Apache License, Version 2.0 (the "License"); you may
 *  not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *  http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS, WITHOUT
 *  WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 *
 *  This file is part of mbed TLS (https://tls.mbed.org)
 */

/*
 * References in the code are to the Thread v1.0 Specification,
 * available to members of the Thread Group http://threadgroup.org/
 */

#if !defined(JHD_TLS_CONFIG_FILE)
#include <tls/jhd_tls_config.h>
#else
#include JHD_TLS_CONFIG_FILE
#endif

#if defined(JHD_TLS_ECJPAKE_C)

#include <tls/jhd_tls_ecjpake.h>

#include <string.h>

#if !defined(JHD_TLS_ECJPAKE_ALT)

/*
 * Convert a jhd_tls_ecjpake_role to identifier string
 */
static const char * const ecjpake_id[] = {
    "client",
    "server"
};

#define ID_MINE     ( ecjpake_id[ ctx->role ] )
#define ID_PEER     ( ecjpake_id[ 1 - ctx->role ] )

/*
 * Initialize context
 */
void jhd_tls_ecjpake_init( jhd_tls_ecjpake_context *ctx )
{
    if( ctx == NULL )
        return;

    ctx->md_info = NULL;
    jhd_tls_ecp_group_init( &ctx->grp );
    ctx->point_format = JHD_TLS_ECP_PF_UNCOMPRESSED;

    jhd_tls_ecp_point_init( &ctx->Xm1 );
    jhd_tls_ecp_point_init( &ctx->Xm2 );
    jhd_tls_ecp_point_init( &ctx->Xp1 );
    jhd_tls_ecp_point_init( &ctx->Xp2 );
    jhd_tls_ecp_point_init( &ctx->Xp  );

    jhd_tls_mpi_init( &ctx->xm1 );
    jhd_tls_mpi_init( &ctx->xm2 );
    jhd_tls_mpi_init( &ctx->s   );
}

/*
 * Free context
 */
void jhd_tls_ecjpake_free( jhd_tls_ecjpake_context *ctx )
{
    if( ctx == NULL )
        return;

    ctx->md_info = NULL;
    jhd_tls_ecp_group_free( &ctx->grp );

    jhd_tls_ecp_point_free( &ctx->Xm1 );
    jhd_tls_ecp_point_free( &ctx->Xm2 );
    jhd_tls_ecp_point_free( &ctx->Xp1 );
    jhd_tls_ecp_point_free( &ctx->Xp2 );
    jhd_tls_ecp_point_free( &ctx->Xp  );

    jhd_tls_mpi_free( &ctx->xm1 );
    jhd_tls_mpi_free( &ctx->xm2 );
    jhd_tls_mpi_free( &ctx->s   );
}

/*
 * Setup context
 */
int jhd_tls_ecjpake_setup( jhd_tls_ecjpake_context *ctx,
                           jhd_tls_ecjpake_role role,
                           jhd_tls_md_type_t hash,
                           jhd_tls_ecp_group_id curve,
                           const unsigned char *secret,
                           size_t len )
{
    int ret;

    ctx->role = role;

    if( ( ctx->md_info = jhd_tls_md_info_from_type( hash ) ) == NULL )
        return( JHD_TLS_ERR_MD_FEATURE_UNAVAILABLE );

    JHD_TLS_MPI_CHK( jhd_tls_ecp_group_load( &ctx->grp, curve ) );

    JHD_TLS_MPI_CHK( jhd_tls_mpi_read_binary( &ctx->s, secret, len ) );

cleanup:
    if( ret != 0 )
        jhd_tls_ecjpake_free( ctx );

    return( ret );
}

/*
 * Check if context is ready for use
 */
int jhd_tls_ecjpake_check( const jhd_tls_ecjpake_context *ctx )
{
    if( ctx->md_info == NULL ||
        ctx->grp.id == JHD_TLS_ECP_DP_NONE ||
        ctx->s.p == NULL )
    {
        return( JHD_TLS_ERR_ECP_BAD_INPUT_DATA );
    }

    return( 0 );
}

/*
 * Write a point plus its length to a buffer
 */
static int ecjpake_write_len_point( unsigned char **p,
                                    const unsigned char *end,
                                    const jhd_tls_ecp_group *grp,
                                    const int pf,
                                    const jhd_tls_ecp_point *P )
{
    int ret;
    size_t len;

    /* Need at least 4 for length plus 1 for point */
    if( end < *p || end - *p < 5 )
        return( JHD_TLS_ERR_ECP_BUFFER_TOO_SMALL );

    ret = jhd_tls_ecp_point_write_binary( grp, P, pf,
                                          &len, *p + 4, end - ( *p + 4 ) );
    if( ret != 0 )
        return( ret );

    (*p)[0] = (unsigned char)( ( len >> 24 ) & 0xFF );
    (*p)[1] = (unsigned char)( ( len >> 16 ) & 0xFF );
    (*p)[2] = (unsigned char)( ( len >>  8 ) & 0xFF );
    (*p)[3] = (unsigned char)( ( len       ) & 0xFF );

    *p += 4 + len;

    return( 0 );
}

/*
 * Size of the temporary buffer for ecjpake_hash:
 * 3 EC points plus their length, plus ID and its length (4 + 6 bytes)
 */
#define ECJPAKE_HASH_BUF_LEN    ( 3 * ( 4 + JHD_TLS_ECP_MAX_PT_LEN ) + 4 + 6 )

/*
 * Compute hash for ZKP (7.4.2.2.2.1)
 */
static int ecjpake_hash( const jhd_tls_md_info_t *md_info,
                         const jhd_tls_ecp_group *grp,
                         const int pf,
                         const jhd_tls_ecp_point *G,
                         const jhd_tls_ecp_point *V,
                         const jhd_tls_ecp_point *X,
                         const char *id,
                         jhd_tls_mpi *h )
{
    int ret;
    unsigned char buf[ECJPAKE_HASH_BUF_LEN];
    unsigned char *p = buf;
    const unsigned char *end = buf + sizeof( buf );
    const size_t id_len = strlen( id );
    unsigned char hash[JHD_TLS_MD_MAX_SIZE];

    /* Write things to temporary buffer */
    JHD_TLS_MPI_CHK( ecjpake_write_len_point( &p, end, grp, pf, G ) );
    JHD_TLS_MPI_CHK( ecjpake_write_len_point( &p, end, grp, pf, V ) );
    JHD_TLS_MPI_CHK( ecjpake_write_len_point( &p, end, grp, pf, X ) );

    if( end - p < 4 )
        return( JHD_TLS_ERR_ECP_BUFFER_TOO_SMALL );

    *p++ = (unsigned char)( ( id_len >> 24 ) & 0xFF );
    *p++ = (unsigned char)( ( id_len >> 16 ) & 0xFF );
    *p++ = (unsigned char)( ( id_len >>  8 ) & 0xFF );
    *p++ = (unsigned char)( ( id_len       ) & 0xFF );

    if( end < p || (size_t)( end - p ) < id_len )
        return( JHD_TLS_ERR_ECP_BUFFER_TOO_SMALL );

    memcpy( p, id, id_len );
    p += id_len;

    /* Compute hash */
    jhd_tls_md( md_info, buf, p - buf, hash );

    /* Turn it into an integer mod n */
    JHD_TLS_MPI_CHK( jhd_tls_mpi_read_binary( h, hash,
                                        jhd_tls_md_get_size( md_info ) ) );
    JHD_TLS_MPI_CHK( jhd_tls_mpi_mod_mpi( h, h, &grp->N ) );

cleanup:
    return( ret );
}

/*
 * Parse a ECShnorrZKP (7.4.2.2.2) and verify it (7.4.2.3.3)
 */
static int ecjpake_zkp_read( const jhd_tls_md_info_t *md_info,
                             const jhd_tls_ecp_group *grp,
                             const int pf,
                             const jhd_tls_ecp_point *G,
                             const jhd_tls_ecp_point *X,
                             const char *id,
                             const unsigned char **p,
                             const unsigned char *end )
{
    int ret;
    jhd_tls_ecp_point V, VV;
    jhd_tls_mpi r, h;
    size_t r_len;

    jhd_tls_ecp_point_init( &V );
    jhd_tls_ecp_point_init( &VV );
    jhd_tls_mpi_init( &r );
    jhd_tls_mpi_init( &h );

    /*
     * struct {
     *     ECPoint V;
     *     opaque r<1..2^8-1>;
     * } ECSchnorrZKP;
     */
    if( end < *p )
        return( JHD_TLS_ERR_ECP_BAD_INPUT_DATA );

    JHD_TLS_MPI_CHK( jhd_tls_ecp_tls_read_point( grp, &V, p, end - *p ) );

    if( end < *p || (size_t)( end - *p ) < 1 )
    {
        ret = JHD_TLS_ERR_ECP_BAD_INPUT_DATA;
        goto cleanup;
    }

    r_len = *(*p)++;

    if( end < *p || (size_t)( end - *p ) < r_len )
    {
        ret = JHD_TLS_ERR_ECP_BAD_INPUT_DATA;
        goto cleanup;
    }

    JHD_TLS_MPI_CHK( jhd_tls_mpi_read_binary( &r, *p, r_len ) );
    *p += r_len;

    /*
     * Verification
     */
    JHD_TLS_MPI_CHK( ecjpake_hash( md_info, grp, pf, G, &V, X, id, &h ) );
    JHD_TLS_MPI_CHK( jhd_tls_ecp_muladd( (jhd_tls_ecp_group *) grp,
                     &VV, &h, X, &r, G ) );

    if( jhd_tls_ecp_point_cmp( &VV, &V ) != 0 )
    {
        ret = JHD_TLS_ERR_ECP_VERIFY_FAILED;
        goto cleanup;
    }

cleanup:
    jhd_tls_ecp_point_free( &V );
    jhd_tls_ecp_point_free( &VV );
    jhd_tls_mpi_free( &r );
    jhd_tls_mpi_free( &h );

    return( ret );
}

/*
 * Generate ZKP (7.4.2.3.2) and write it as ECSchnorrZKP (7.4.2.2.2)
 */
static int ecjpake_zkp_write( const jhd_tls_md_info_t *md_info,
                              const jhd_tls_ecp_group *grp,
                              const int pf,
                              const jhd_tls_ecp_point *G,
                              const jhd_tls_mpi *x,
                              const jhd_tls_ecp_point *X,
                              const char *id,
                              unsigned char **p,
                              const unsigned char *end,
                              int (*f_rng)(void *, unsigned char *, size_t),
                              void *p_rng )
{
    int ret;
    jhd_tls_ecp_point V;
    jhd_tls_mpi v;
    jhd_tls_mpi h; /* later recycled to hold r */
    size_t len;

    if( end < *p )
        return( JHD_TLS_ERR_ECP_BUFFER_TOO_SMALL );

    jhd_tls_ecp_point_init( &V );
    jhd_tls_mpi_init( &v );
    jhd_tls_mpi_init( &h );

    /* Compute signature */
    JHD_TLS_MPI_CHK( jhd_tls_ecp_gen_keypair_base( (jhd_tls_ecp_group *) grp,
                                                   G, &v, &V, f_rng, p_rng ) );
    JHD_TLS_MPI_CHK( ecjpake_hash( md_info, grp, pf, G, &V, X, id, &h ) );
    JHD_TLS_MPI_CHK( jhd_tls_mpi_mul_mpi( &h, &h, x ) ); /* x*h */
    JHD_TLS_MPI_CHK( jhd_tls_mpi_sub_mpi( &h, &v, &h ) ); /* v - x*h */
    JHD_TLS_MPI_CHK( jhd_tls_mpi_mod_mpi( &h, &h, &grp->N ) ); /* r */

    /* Write it out */
    JHD_TLS_MPI_CHK( jhd_tls_ecp_tls_write_point( grp, &V,
                pf, &len, *p, end - *p ) );
    *p += len;

    len = jhd_tls_mpi_size( &h ); /* actually r */
    if( end < *p || (size_t)( end - *p ) < 1 + len || len > 255 )
    {
        ret = JHD_TLS_ERR_ECP_BUFFER_TOO_SMALL;
        goto cleanup;
    }

    *(*p)++ = (unsigned char)( len & 0xFF );
    JHD_TLS_MPI_CHK( jhd_tls_mpi_write_binary( &h, *p, len ) ); /* r */
    *p += len;

cleanup:
    jhd_tls_ecp_point_free( &V );
    jhd_tls_mpi_free( &v );
    jhd_tls_mpi_free( &h );

    return( ret );
}

/*
 * Parse a ECJPAKEKeyKP (7.4.2.2.1) and check proof
 * Output: verified public key X
 */
static int ecjpake_kkp_read( const jhd_tls_md_info_t *md_info,
                             const jhd_tls_ecp_group *grp,
                             const int pf,
                             const jhd_tls_ecp_point *G,
                             jhd_tls_ecp_point *X,
                             const char *id,
                             const unsigned char **p,
                             const unsigned char *end )
{
    int ret;

    if( end < *p )
        return( JHD_TLS_ERR_ECP_BAD_INPUT_DATA );

    /*
     * struct {
     *     ECPoint X;
     *     ECSchnorrZKP zkp;
     * } ECJPAKEKeyKP;
     */
    JHD_TLS_MPI_CHK( jhd_tls_ecp_tls_read_point( grp, X, p, end - *p ) );
    if( jhd_tls_ecp_is_zero( X ) )
    {
        ret = JHD_TLS_ERR_ECP_INVALID_KEY;
        goto cleanup;
    }

    JHD_TLS_MPI_CHK( ecjpake_zkp_read( md_info, grp, pf, G, X, id, p, end ) );

cleanup:
    return( ret );
}

/*
 * Generate an ECJPAKEKeyKP
 * Output: the serialized structure, plus private/public key pair
 */
static int ecjpake_kkp_write( const jhd_tls_md_info_t *md_info,
                              const jhd_tls_ecp_group *grp,
                              const int pf,
                              const jhd_tls_ecp_point *G,
                              jhd_tls_mpi *x,
                              jhd_tls_ecp_point *X,
                              const char *id,
                              unsigned char **p,
                              const unsigned char *end,
                              int (*f_rng)(void *, unsigned char *, size_t),
                              void *p_rng )
{
    int ret;
    size_t len;

    if( end < *p )
        return( JHD_TLS_ERR_ECP_BUFFER_TOO_SMALL );

    /* Generate key (7.4.2.3.1) and write it out */
    JHD_TLS_MPI_CHK( jhd_tls_ecp_gen_keypair_base( (jhd_tls_ecp_group *) grp, G, x, X,
                                                   f_rng, p_rng ) );
    JHD_TLS_MPI_CHK( jhd_tls_ecp_tls_write_point( grp, X,
                pf, &len, *p, end - *p ) );
    *p += len;

    /* Generate and write proof */
    JHD_TLS_MPI_CHK( ecjpake_zkp_write( md_info, grp, pf, G, x, X, id,
                                        p, end, f_rng, p_rng ) );

cleanup:
    return( ret );
}

/*
 * Read a ECJPAKEKeyKPPairList (7.4.2.3) and check proofs
 * Ouputs: verified peer public keys Xa, Xb
 */
static int ecjpake_kkpp_read( const jhd_tls_md_info_t *md_info,
                              const jhd_tls_ecp_group *grp,
                              const int pf,
                              const jhd_tls_ecp_point *G,
                              jhd_tls_ecp_point *Xa,
                              jhd_tls_ecp_point *Xb,
                              const char *id,
                              const unsigned char *buf,
                              size_t len )
{
    int ret;
    const unsigned char *p = buf;
    const unsigned char *end = buf + len;

    /*
     * struct {
     *     ECJPAKEKeyKP ecjpake_key_kp_pair_list[2];
     * } ECJPAKEKeyKPPairList;
     */
    JHD_TLS_MPI_CHK( ecjpake_kkp_read( md_info, grp, pf, G, Xa, id, &p, end ) );
    JHD_TLS_MPI_CHK( ecjpake_kkp_read( md_info, grp, pf, G, Xb, id, &p, end ) );

    if( p != end )
        ret = JHD_TLS_ERR_ECP_BAD_INPUT_DATA;

cleanup:
    return( ret );
}

/*
 * Generate a ECJPAKEKeyKPPairList
 * Outputs: the serialized structure, plus two private/public key pairs
 */
static int ecjpake_kkpp_write( const jhd_tls_md_info_t *md_info,
                               const jhd_tls_ecp_group *grp,
                               const int pf,
                               const jhd_tls_ecp_point *G,
                               jhd_tls_mpi *xm1,
                               jhd_tls_ecp_point *Xa,
                               jhd_tls_mpi *xm2,
                               jhd_tls_ecp_point *Xb,
                               const char *id,
                               unsigned char *buf,
                               size_t len,
                               size_t *olen,
                               int (*f_rng)(void *, unsigned char *, size_t),
                               void *p_rng )
{
    int ret;
    unsigned char *p = buf;
    const unsigned char *end = buf + len;

    JHD_TLS_MPI_CHK( ecjpake_kkp_write( md_info, grp, pf, G, xm1, Xa, id,
                &p, end, f_rng, p_rng ) );
    JHD_TLS_MPI_CHK( ecjpake_kkp_write( md_info, grp, pf, G, xm2, Xb, id,
                &p, end, f_rng, p_rng ) );

    *olen = p - buf;

cleanup:
    return( ret );
}

/*
 * Read and process the first round message
 */
int jhd_tls_ecjpake_read_round_one( jhd_tls_ecjpake_context *ctx,
                                    const unsigned char *buf,
                                    size_t len )
{
    return( ecjpake_kkpp_read( ctx->md_info, &ctx->grp, ctx->point_format,
                               &ctx->grp.G,
                               &ctx->Xp1, &ctx->Xp2, ID_PEER,
                               buf, len ) );
}

/*
 * Generate and write the first round message
 */
int jhd_tls_ecjpake_write_round_one( jhd_tls_ecjpake_context *ctx,
                            unsigned char *buf, size_t len, size_t *olen,
                            int (*f_rng)(void *, unsigned char *, size_t),
                            void *p_rng )
{
    return( ecjpake_kkpp_write( ctx->md_info, &ctx->grp, ctx->point_format,
                                &ctx->grp.G,
                                &ctx->xm1, &ctx->Xm1, &ctx->xm2, &ctx->Xm2,
                                ID_MINE, buf, len, olen, f_rng, p_rng ) );
}

/*
 * Compute the sum of three points R = A + B + C
 */
static int ecjpake_ecp_add3( jhd_tls_ecp_group *grp, jhd_tls_ecp_point *R,
                             const jhd_tls_ecp_point *A,
                             const jhd_tls_ecp_point *B,
                             const jhd_tls_ecp_point *C )
{
    int ret;
    jhd_tls_mpi one;

    jhd_tls_mpi_init( &one );

    JHD_TLS_MPI_CHK( jhd_tls_mpi_lset( &one, 1 ) );
    JHD_TLS_MPI_CHK( jhd_tls_ecp_muladd( grp, R, &one, A, &one, B ) );
    JHD_TLS_MPI_CHK( jhd_tls_ecp_muladd( grp, R, &one, R, &one, C ) );

cleanup:
    jhd_tls_mpi_free( &one );

    return( ret );
}

/*
 * Read and process second round message (C: 7.4.2.5, S: 7.4.2.6)
 */
int jhd_tls_ecjpake_read_round_two( jhd_tls_ecjpake_context *ctx,
                                            const unsigned char *buf,
                                            size_t len )
{
    int ret;
    const unsigned char *p = buf;
    const unsigned char *end = buf + len;
    jhd_tls_ecp_group grp;
    jhd_tls_ecp_point G;    /* C: GB, S: GA */

    jhd_tls_ecp_group_init( &grp );
    jhd_tls_ecp_point_init( &G );

    /*
     * Server: GA = X3  + X4  + X1      (7.4.2.6.1)
     * Client: GB = X1  + X2  + X3      (7.4.2.5.1)
     * Unified: G = Xm1 + Xm2 + Xp1
     * We need that before parsing in order to check Xp as we read it
     */
    JHD_TLS_MPI_CHK( ecjpake_ecp_add3( &ctx->grp, &G,
                                       &ctx->Xm1, &ctx->Xm2, &ctx->Xp1 ) );

    /*
     * struct {
     *     ECParameters curve_params;   // only client reading server msg
     *     ECJPAKEKeyKP ecjpake_key_kp;
     * } Client/ServerECJPAKEParams;
     */
    if( ctx->role == JHD_TLS_ECJPAKE_CLIENT )
    {
        JHD_TLS_MPI_CHK( jhd_tls_ecp_tls_read_group( &grp, &p, len ) );
        if( grp.id != ctx->grp.id )
        {
            ret = JHD_TLS_ERR_ECP_FEATURE_UNAVAILABLE;
            goto cleanup;
        }
    }

    JHD_TLS_MPI_CHK( ecjpake_kkp_read( ctx->md_info, &ctx->grp,
                            ctx->point_format,
                            &G, &ctx->Xp, ID_PEER, &p, end ) );

    if( p != end )
    {
        ret = JHD_TLS_ERR_ECP_BAD_INPUT_DATA;
        goto cleanup;
    }

cleanup:
    jhd_tls_ecp_group_free( &grp );
    jhd_tls_ecp_point_free( &G );

    return( ret );
}

/*
 * Compute R = +/- X * S mod N, taking care not to leak S
 */
static int ecjpake_mul_secret( jhd_tls_mpi *R, int sign,
                               const jhd_tls_mpi *X,
                               const jhd_tls_mpi *S,
                               const jhd_tls_mpi *N,
                               int (*f_rng)(void *, unsigned char *, size_t),
                               void *p_rng )
{
    int ret;
    jhd_tls_mpi b; /* Blinding value, then s + N * blinding */

    jhd_tls_mpi_init( &b );

    /* b = s + rnd-128-bit * N */
    JHD_TLS_MPI_CHK( jhd_tls_mpi_fill_random( &b, 16, f_rng, p_rng ) );
    JHD_TLS_MPI_CHK( jhd_tls_mpi_mul_mpi( &b, &b, N ) );
    JHD_TLS_MPI_CHK( jhd_tls_mpi_add_mpi( &b, &b, S ) );

    /* R = sign * X * b mod N */
    JHD_TLS_MPI_CHK( jhd_tls_mpi_mul_mpi( R, X, &b ) );
    R->s *= sign;
    JHD_TLS_MPI_CHK( jhd_tls_mpi_mod_mpi( R, R, N ) );

cleanup:
    jhd_tls_mpi_free( &b );

    return( ret );
}

/*
 * Generate and write the second round message (S: 7.4.2.5, C: 7.4.2.6)
 */
int jhd_tls_ecjpake_write_round_two( jhd_tls_ecjpake_context *ctx,
                            unsigned char *buf, size_t len, size_t *olen,
                            int (*f_rng)(void *, unsigned char *, size_t),
                            void *p_rng )
{
    int ret;
    jhd_tls_ecp_point G;    /* C: GA, S: GB */
    jhd_tls_ecp_point Xm;   /* C: Xc, S: Xs */
    jhd_tls_mpi xm;         /* C: xc, S: xs */
    unsigned char *p = buf;
    const unsigned char *end = buf + len;
    size_t ec_len;

    jhd_tls_ecp_point_init( &G );
    jhd_tls_ecp_point_init( &Xm );
    jhd_tls_mpi_init( &xm );

    /*
     * First generate private/public key pair (S: 7.4.2.5.1, C: 7.4.2.6.1)
     *
     * Client:  GA = X1  + X3  + X4  | xs = x2  * s | Xc = xc * GA
     * Server:  GB = X3  + X1  + X2  | xs = x4  * s | Xs = xs * GB
     * Unified: G  = Xm1 + Xp1 + Xp2 | xm = xm2 * s | Xm = xm * G
     */
    JHD_TLS_MPI_CHK( ecjpake_ecp_add3( &ctx->grp, &G,
                                       &ctx->Xp1, &ctx->Xp2, &ctx->Xm1 ) );
    JHD_TLS_MPI_CHK( ecjpake_mul_secret( &xm, 1, &ctx->xm2, &ctx->s,
                                         &ctx->grp.N, f_rng, p_rng ) );
    JHD_TLS_MPI_CHK( jhd_tls_ecp_mul( &ctx->grp, &Xm, &xm, &G, f_rng, p_rng ) );

    /*
     * Now write things out
     *
     * struct {
     *     ECParameters curve_params;   // only server writing its message
     *     ECJPAKEKeyKP ecjpake_key_kp;
     * } Client/ServerECJPAKEParams;
     */
    if( ctx->role == JHD_TLS_ECJPAKE_SERVER )
    {
        if( end < p )
        {
            ret = JHD_TLS_ERR_ECP_BUFFER_TOO_SMALL;
            goto cleanup;
        }
        JHD_TLS_MPI_CHK( jhd_tls_ecp_tls_write_group( &ctx->grp, &ec_len,
                                                      p, end - p ) );
        p += ec_len;
    }

    if( end < p )
    {
        ret = JHD_TLS_ERR_ECP_BUFFER_TOO_SMALL;
        goto cleanup;
    }
    JHD_TLS_MPI_CHK( jhd_tls_ecp_tls_write_point( &ctx->grp, &Xm,
                     ctx->point_format, &ec_len, p, end - p ) );
    p += ec_len;

    JHD_TLS_MPI_CHK( ecjpake_zkp_write( ctx->md_info, &ctx->grp,
                                        ctx->point_format,
                                        &G, &xm, &Xm, ID_MINE,
                                        &p, end, f_rng, p_rng ) );

    *olen = p - buf;

cleanup:
    jhd_tls_ecp_point_free( &G );
    jhd_tls_ecp_point_free( &Xm );
    jhd_tls_mpi_free( &xm );

    return( ret );
}

/*
 * Derive PMS (7.4.2.7 / 7.4.2.8)
 */
int jhd_tls_ecjpake_derive_secret( jhd_tls_ecjpake_context *ctx,
                            unsigned char *buf, size_t len, size_t *olen,
                            int (*f_rng)(void *, unsigned char *, size_t),
                            void *p_rng )
{
    int ret;
    jhd_tls_ecp_point K;
    jhd_tls_mpi m_xm2_s, one;
    unsigned char kx[JHD_TLS_ECP_MAX_BYTES];
    size_t x_bytes;

    *olen = jhd_tls_md_get_size( ctx->md_info );
    if( len < *olen )
        return( JHD_TLS_ERR_ECP_BUFFER_TOO_SMALL );

    jhd_tls_ecp_point_init( &K );
    jhd_tls_mpi_init( &m_xm2_s );
    jhd_tls_mpi_init( &one );

    JHD_TLS_MPI_CHK( jhd_tls_mpi_lset( &one, 1 ) );

    /*
     * Client:  K = ( Xs - X4  * x2  * s ) * x2
     * Server:  K = ( Xc - X2  * x4  * s ) * x4
     * Unified: K = ( Xp - Xp2 * xm2 * s ) * xm2
     */
    JHD_TLS_MPI_CHK( ecjpake_mul_secret( &m_xm2_s, -1, &ctx->xm2, &ctx->s,
                                         &ctx->grp.N, f_rng, p_rng ) );
    JHD_TLS_MPI_CHK( jhd_tls_ecp_muladd( &ctx->grp, &K,
                                         &one, &ctx->Xp,
                                         &m_xm2_s, &ctx->Xp2 ) );
    JHD_TLS_MPI_CHK( jhd_tls_ecp_mul( &ctx->grp, &K, &ctx->xm2, &K,
                                      f_rng, p_rng ) );

    /* PMS = SHA-256( K.X ) */
    x_bytes = ( ctx->grp.pbits + 7 ) / 8;
    JHD_TLS_MPI_CHK( jhd_tls_mpi_write_binary( &K.X, kx, x_bytes ) );
    JHD_TLS_MPI_CHK( jhd_tls_md( ctx->md_info, kx, x_bytes, buf ) );

cleanup:
    jhd_tls_ecp_point_free( &K );
    jhd_tls_mpi_free( &m_xm2_s );
    jhd_tls_mpi_free( &one );

    return( ret );
}

#undef ID_MINE
#undef ID_PEER

#endif /* ! JHD_TLS_ECJPAKE_ALT */

#if defined(JHD_TLS_SELF_TEST)

#if defined(JHD_TLS_PLATFORM_C)
#include <tls/jhd_tls_platform.h>
#else
#include <stdio.h>
#define jhd_tls_printf     printf
#endif

#if !defined(JHD_TLS_ECP_DP_SECP256R1_ENABLED) || \
    !defined(JHD_TLS_SHA256_C)
int jhd_tls_ecjpake_self_test( int verbose )
{
    (void) verbose;
    return( 0 );
}
#else

static const unsigned char ecjpake_test_password[] = {
    0x74, 0x68, 0x72, 0x65, 0x61, 0x64, 0x6a, 0x70, 0x61, 0x6b, 0x65, 0x74,
    0x65, 0x73, 0x74
};

static const unsigned char ecjpake_test_x1[] = {
    0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c,
    0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
    0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x21
};

static const unsigned char ecjpake_test_x2[] = {
    0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c,
    0x6d, 0x6e, 0x6f, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78,
    0x79, 0x7a, 0x7b, 0x7c, 0x7d, 0x7e, 0x7f, 0x81
};

static const unsigned char ecjpake_test_x3[] = {
    0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c,
    0x6d, 0x6e, 0x6f, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78,
    0x79, 0x7a, 0x7b, 0x7c, 0x7d, 0x7e, 0x7f, 0x81
};

static const unsigned char ecjpake_test_x4[] = {
    0xc1, 0xc2, 0xc3, 0xc4, 0xc5, 0xc6, 0xc7, 0xc8, 0xc9, 0xca, 0xcb, 0xcc,
    0xcd, 0xce, 0xcf, 0xd0, 0xd1, 0xd2, 0xd3, 0xd4, 0xd5, 0xd6, 0xd7, 0xd8,
    0xd9, 0xda, 0xdb, 0xdc, 0xdd, 0xde, 0xdf, 0xe1
};

static const unsigned char ecjpake_test_cli_one[] = {
    
};

static const unsigned char ecjpake_test_srv_one[] = {
    
};

static const unsigned char ecjpake_test_srv_two[] = {
    
};

static const unsigned char ecjpake_test_cli_two[] = {
    
};

static const unsigned char ecjpake_test_pms[] = {
    0xf3, 0xd4, 0x7f, 0x59, 0x98, 0x44, 0xdb, 0x92, 0xa5, 0x69, 0xbb, 0xe7,
    0x98, 0x1e, 0x39, 0xd9, 0x31, 0xfd, 0x74, 0x3b, 0xf2, 0x2e, 0x98, 0xf9,
    0xb4, 0x38, 0xf7, 0x19, 0xd3, 0xc4, 0xf3, 0x51
};

/* Load my private keys and generate the correponding public keys */
static int ecjpake_test_load( jhd_tls_ecjpake_context *ctx,
                              const unsigned char *xm1, size_t len1,
                              const unsigned char *xm2, size_t len2 )
{
    int ret;

    JHD_TLS_MPI_CHK( jhd_tls_mpi_read_binary( &ctx->xm1, xm1, len1 ) );
    JHD_TLS_MPI_CHK( jhd_tls_mpi_read_binary( &ctx->xm2, xm2, len2 ) );
    JHD_TLS_MPI_CHK( jhd_tls_ecp_mul( &ctx->grp, &ctx->Xm1, &ctx->xm1,
                                      &ctx->grp.G, NULL, NULL ) );
    JHD_TLS_MPI_CHK( jhd_tls_ecp_mul( &ctx->grp, &ctx->Xm2, &ctx->xm2,
                                      &ctx->grp.G, NULL, NULL ) );

cleanup:
    return( ret );
}

/* For tests we don't need a secure RNG;
 * use the LGC from Numerical Recipes for simplicity */
static int ecjpake_lgc( void *p, unsigned char *out, size_t len )
{
    static uint32_t x = 42;
    (void) p;

    while( len > 0 )
    {
        size_t use_len = len > 4 ? 4 : len;
        x = 1664525 * x + 1013904223;
        memcpy( out, &x, use_len );
        out += use_len;
        len -= use_len;
    }

    return( 0 );
}

#define TEST_ASSERT( x )    \
    do {                    \
        if( x )             \
            ret = 0;        \
        else                \
        {                   \
            ret = 1;        \
            goto cleanup;   \
        }                   \
    } while( 0 )

/*
 * Checkup routine
 */
int jhd_tls_ecjpake_self_test( int verbose )
{
    int ret;
    jhd_tls_ecjpake_context cli;
    jhd_tls_ecjpake_context srv;
    unsigned char buf[512], pms[32];
    size_t len, pmslen;

    jhd_tls_ecjpake_init( &cli );
    jhd_tls_ecjpake_init( &srv );

    if( verbose != 0 )
        jhd_tls_printf( "  ECJPAKE test #0 (setup): " );

    TEST_ASSERT( jhd_tls_ecjpake_setup( &cli, JHD_TLS_ECJPAKE_CLIENT,
                    JHD_TLS_MD_SHA256, JHD_TLS_ECP_DP_SECP256R1,
                    ecjpake_test_password,
            sizeof( ecjpake_test_password ) ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_setup( &srv, JHD_TLS_ECJPAKE_SERVER,
                    JHD_TLS_MD_SHA256, JHD_TLS_ECP_DP_SECP256R1,
                    ecjpake_test_password,
            sizeof( ecjpake_test_password ) ) == 0 );

    if( verbose != 0 )
        jhd_tls_printf( "passed\n" );

    if( verbose != 0 )
        jhd_tls_printf( "  ECJPAKE test #1 (random handshake): " );

    TEST_ASSERT( jhd_tls_ecjpake_write_round_one( &cli,
                 buf, sizeof( buf ), &len, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_read_round_one( &srv, buf, len ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_write_round_one( &srv,
                 buf, sizeof( buf ), &len, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_read_round_one( &cli, buf, len ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_write_round_two( &srv,
                 buf, sizeof( buf ), &len, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_read_round_two( &cli, buf, len ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_derive_secret( &cli,
                 pms, sizeof( pms ), &pmslen, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_write_round_two( &cli,
                 buf, sizeof( buf ), &len, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_read_round_two( &srv, buf, len ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_derive_secret( &srv,
                 buf, sizeof( buf ), &len, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( len == pmslen );
    TEST_ASSERT( memcmp( buf, pms, len ) == 0 );

    if( verbose != 0 )
        jhd_tls_printf( "passed\n" );

    if( verbose != 0 )
        jhd_tls_printf( "  ECJPAKE test #2 (reference handshake): " );

    /* Simulate generation of round one */
    JHD_TLS_MPI_CHK( ecjpake_test_load( &cli,
                ecjpake_test_x1, sizeof( ecjpake_test_x1 ),
                ecjpake_test_x2, sizeof( ecjpake_test_x2 ) ) );

    JHD_TLS_MPI_CHK( ecjpake_test_load( &srv,
                ecjpake_test_x3, sizeof( ecjpake_test_x3 ),
                ecjpake_test_x4, sizeof( ecjpake_test_x4 ) ) );

    /* Read round one */
    TEST_ASSERT( jhd_tls_ecjpake_read_round_one( &srv,
                                    ecjpake_test_cli_one,
                            sizeof( ecjpake_test_cli_one ) ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_read_round_one( &cli,
                                    ecjpake_test_srv_one,
                            sizeof( ecjpake_test_srv_one ) ) == 0 );

    /* Skip generation of round two, read round two */
    TEST_ASSERT( jhd_tls_ecjpake_read_round_two( &cli,
                                    ecjpake_test_srv_two,
                            sizeof( ecjpake_test_srv_two ) ) == 0 );

    TEST_ASSERT( jhd_tls_ecjpake_read_round_two( &srv,
                                    ecjpake_test_cli_two,
                            sizeof( ecjpake_test_cli_two ) ) == 0 );

    /* Server derives PMS */
    TEST_ASSERT( jhd_tls_ecjpake_derive_secret( &srv,
                 buf, sizeof( buf ), &len, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( len == sizeof( ecjpake_test_pms ) );
    TEST_ASSERT( memcmp( buf, ecjpake_test_pms, len ) == 0 );

    memset( buf, 0, len ); /* Avoid interferences with next step */

    /* Client derives PMS */
    TEST_ASSERT( jhd_tls_ecjpake_derive_secret( &cli,
                 buf, sizeof( buf ), &len, ecjpake_lgc, NULL ) == 0 );

    TEST_ASSERT( len == sizeof( ecjpake_test_pms ) );
    TEST_ASSERT( memcmp( buf, ecjpake_test_pms, len ) == 0 );

    if( verbose != 0 )
        jhd_tls_printf( "passed\n" );

cleanup:
    jhd_tls_ecjpake_free( &cli );
    jhd_tls_ecjpake_free( &srv );

    if( ret != 0 )
    {
        if( verbose != 0 )
            jhd_tls_printf( "failed\n" );

        ret = 1;
    }

    if( verbose != 0 )
        jhd_tls_printf( "\n" );

    return( ret );
}

#undef TEST_ASSERT

#endif /* JHD_TLS_ECP_DP_SECP256R1_ENABLED && JHD_TLS_SHA256_C */

#endif /* JHD_TLS_SELF_TEST */

#endif /* JHD_TLS_ECJPAKE_C */
