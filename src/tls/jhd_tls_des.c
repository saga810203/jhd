...
{
    memset( ctx, 0, sizeof( jhd_tls_des_context ) );
}
...
{
    memset( ctx, 0, sizeof( jhd_tls_des3_context ) );
}
...
void jhd_tls_des3_free( jhd_tls_des3_context *ctx )
{
    ...
    jhd_tls_platform_zeroize( ctx, sizeof( jhd_tls_des3_context ) );
}
...
static const unsigned char weak_key_table[WEAK_KEY_COUNT][JHD_TLS_DES_KEY_SIZE] =
{
                      ...
                      const unsigned char key[JHD_TLS_DES_KEY_SIZE * 2] )
{
    ...
    jhd_tls_platform_zeroize( sk,  sizeof( sk ) );
                    ...
                    unsigned char *output )
{
    ...
    if( length % 8 )
        return( JHD_TLS_ERR_DES_INVALID_INPUT_LENGTH );

    if( mode == JHD_TLS_DES_ENCRYPT )
    {
        while( length > 0 )
        {
            for( i = 0; i < 8; i++ )
            ...
            jhd_tls_des_crypt_ecb( ctx, output, output );
            memcpy( iv, output, 8 );
    ...
    else /* JHD_TLS_DES_DECRYPT */
    {
        ...
        {
            memcpy( temp, input, 8 );
            jhd_tls_des_crypt_ecb( ctx, input, output );
                     ...
                     unsigned char *output )
{
    ...
    if( mode == JHD_TLS_DES_ENCRYPT )
    {
        while( length > 0 )
        {
            ...
            jhd_tls_des3_crypt_ecb( ctx, output, output );
            memcpy( iv, output, 8 );
    ...
    else /* JHD_TLS_DES_DECRYPT */
    {
        ...
        {
            memcpy( temp, input, 8 );
            jhd_tls_des3_crypt_ecb( ctx, input, output );
...
int jhd_tls_des_self_test( int verbose )
{
    ...
    jhd_tls_des_init( &ctx );
    jhd_tls_des3_init( &ctx3 );
    /*
     * ECB mode
     */
    for( i = 0; i < 6; i++ )
    {
        ...
        if( verbose != 0 )
            jhd_tls_printf( "  DES%c-ECB-%3d (%s): ",
                             ( u == 0 ) ? ' ' : '3', 56 + u * 56,
                             ( v == JHD_TLS_DES_DECRYPT ) ? "dec" : "enc" );
        ...
        {
        case 0:
            jhd_tls_des_setkey_dec( &ctx, des3_test_keys );
            break;

        case 1:
            jhd_tls_des_setkey_enc( &ctx, des3_test_keys );
            break;

        case 2:
            jhd_tls_des3_set2key_dec( &ctx3, des3_test_keys );
            break;

        case 3:
            jhd_tls_des3_set2key_enc( &ctx3, des3_test_keys );
            break;

        case 4:
            jhd_tls_des3_set3key_dec( &ctx3, des3_test_keys );
            break;

        case 5:
            jhd_tls_des3_set3key_enc( &ctx3, des3_test_keys );
            break;
        ...
        {
            if( u == 0 )
                jhd_tls_des_crypt_ecb( &ctx, buf, buf );
            else
                jhd_tls_des3_crypt_ecb( &ctx3, buf, buf );
        }

        if( ( v == JHD_TLS_DES_DECRYPT &&
                memcmp( buf, des3_test_ecb_dec[u], 8 ) != 0 ) ||
            ( v != JHD_TLS_DES_DECRYPT &&
...
exit:
    jhd_tls_des_free( &ctx );
    jhd_tls_des3_free( &ctx3 );
