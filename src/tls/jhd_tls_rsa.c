                        ...
                        const jhd_tls_mpi *D, const jhd_tls_mpi *E )
{
    ...
    if( ( N != NULL && ( ret = jhd_tls_mpi_copy( &ctx->N, N ) ) != 0 ) ||
        ( P != NULL && ( ret = jhd_tls_mpi_copy( &ctx->P, P ) ) != 0 ) ||
        ( Q != NULL && ( ret = jhd_tls_mpi_copy( &ctx->Q, Q ) ) != 0 ) ||
        ( D != NULL && ( ret = jhd_tls_mpi_copy( &ctx->D, D ) ) != 0 ) ||
                              ...
                              int blinding_needed )
{
    ...
    if( ctx->len != jhd_tls_mpi_size( &ctx->N ) ||
        ctx->len > JHD_TLS_MPI_MAX_SIZE )
    {
    ...
    if( jhd_tls_mpi_cmp_int( &ctx->N, 0 ) <= 0 ||
        jhd_tls_mpi_get_bit( &ctx->N, 0 ) == 0  )
    {
        return( JHD_TLS_ERR_RSA_BAD_INPUT_DATA );
    }
...
#else
    if( is_priv &&
        ( jhd_tls_mpi_cmp_int( &ctx->DP, 0 ) <= 0 ||
          jhd_tls_mpi_cmp_int( &ctx->DQ, 0 ) <= 0  ) )
    {
        return( JHD_TLS_ERR_RSA_BAD_INPUT_DATA );
    }
#endif /* JHD_TLS_RSA_NO_CRT */
...
#if !defined(JHD_TLS_RSA_NO_CRT)
    if( is_priv &&
        jhd_tls_mpi_cmp_int( &ctx->QP, 0 ) <= 0 )
    {
        return( JHD_TLS_ERR_RSA_BAD_INPUT_DATA );
    }
#endif
...
int jhd_tls_rsa_complete( jhd_tls_rsa_context *ctx )
{
    ...
    {
        if( ( ret = jhd_tls_mpi_mul_mpi( &ctx->N, &ctx->P,
                                         &ctx->Q ) ) != 0 )
        {
            return( JHD_TLS_ERR_RSA_BAD_INPUT_DATA + ret );
        }
    ...
    if( pq_missing )
    {
        ret = jhd_tls_rsa_deduce_primes( &ctx->N, &ctx->E, &ctx->D,
                                         &ctx->P, &ctx->Q );
        if( ret != 0 )
            return( JHD_TLS_ERR_RSA_BAD_INPUT_DATA + ret );
    ...
    {
        if( ( ret = jhd_tls_rsa_deduce_private_exponent( &ctx->P,
                                                         &ctx->Q,
                                                         &ctx->E,
                                                         &ctx->D ) ) != 0 )
        {
    ...
    if( is_priv )
    {
        ret = jhd_tls_rsa_deduce_crt( &ctx->P,  &ctx->Q,  &ctx->D,
                            ...
                            unsigned char *E, size_t E_len )
{
    ...
    const int is_priv =
        jhd_tls_mpi_cmp_int( &ctx->N, 0 ) != 0 &&
        jhd_tls_mpi_cmp_int( &ctx->P, 0 ) != 0 &&
        jhd_tls_mpi_cmp_int( &ctx->Q, 0 ) != 0 &&
        jhd_tls_mpi_cmp_int( &ctx->D, 0 ) != 0 &&
        jhd_tls_mpi_cmp_int( &ctx->E, 0 ) != 0;
                        ...
                        jhd_tls_mpi *D, jhd_tls_mpi *E )
{
    ...
    if( ( N != NULL && ( ret = jhd_tls_mpi_copy( N, &ctx->N ) ) != 0 ) ||
        ( P != NULL && ( ret = jhd_tls_mpi_copy( P, &ctx->P ) ) != 0 ) ||
        ( Q != NULL && ( ret = jhd_tls_mpi_copy( Q, &ctx->Q ) ) != 0 ) ||
        ( D != NULL && ( ret = jhd_tls_mpi_copy( D, &ctx->D ) ) != 0 ) ||
                            ...
                            jhd_tls_mpi *DP, jhd_tls_mpi *DQ, jhd_tls_mpi *QP )
{
    ...
    if( ( DP != NULL && ( ret = jhd_tls_mpi_copy( DP, &ctx->DP ) ) != 0 ) ||
        ( DQ != NULL && ( ret = jhd_tls_mpi_copy( DQ, &ctx->DQ ) ) != 0 ) ||
        ( QP != NULL && ( ret = jhd_tls_mpi_copy( QP, &ctx->QP ) ) != 0 ) )
    {
        return( JHD_TLS_ERR_RSA_BAD_INPUT_DATA + ret );
    }
#else
                 ...
                 unsigned int nbits, int exponent )
{
     ...
     * QP = Q^-1 mod P
     */
    JHD_TLS_MPI_CHK( jhd_tls_rsa_deduce_crt( &ctx->P, &ctx->Q, &ctx->D,
                                             &ctx->DP, &ctx->DQ, &ctx->QP ) );
#endif /* JHD_TLS_RSA_NO_CRT */
...
int jhd_tls_rsa_check_pubkey( const jhd_tls_rsa_context *ctx )
{
    ...
    if( jhd_tls_mpi_get_bit( &ctx->E, 0 ) == 0 ||
        jhd_tls_mpi_bitlen( &ctx->E )     < 2  ||
        jhd_tls_mpi_cmp_mpi( &ctx->E, &ctx->N ) >= 0 )
    {
        return( JHD_TLS_ERR_RSA_KEY_CHECK_FAILED );
    }
...
{
    if( jhd_tls_rsa_check_pubkey( ctx ) != 0 ||
        rsa_check_context( ctx, 1 /* private */, 1 /* blinding */ ) != 0 )
    {
        return( JHD_TLS_ERR_RSA_KEY_CHECK_FAILED );
    }

    if( jhd_tls_rsa_validate_params( &ctx->N, &ctx->P, &ctx->Q,
...
#if !defined(JHD_TLS_RSA_NO_CRT)
    else if( jhd_tls_rsa_validate_crt( &ctx->P, &ctx->Q, &ctx->D,
...
{
    if( jhd_tls_rsa_check_pubkey( pub )  != 0 ||
        jhd_tls_rsa_check_privkey( prv ) != 0 )
    {
        return( JHD_TLS_ERR_RSA_KEY_CHECK_FAILED );
    }

    if( jhd_tls_mpi_cmp_mpi( &pub->N, &prv->N ) != 0 ||
        jhd_tls_mpi_cmp_mpi( &pub->E, &prv->E ) != 0 )
    {
        return( JHD_TLS_ERR_RSA_KEY_CHECK_FAILED );
    }
...
#if defined(JHD_TLS_THREADING_C)
    if( ( ret = jhd_tls_mutex_lock( &ctx->mutex ) ) != 0 )
        return( ret );
#endif
...
cleanup:
#if defined(JHD_TLS_THREADING_C)
    if( jhd_tls_mutex_unlock( &ctx->mutex ) != 0 )
                 ...
                 int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    ...
    do {
        ...
        JHD_TLS_MPI_CHK( jhd_tls_mpi_fill_random( &ctx->Vf, ctx->len - 1, f_rng, p_rng ) );
                 ...
                 unsigned char *output )
{
...
#if !defined(JHD_TLS_RSA_NO_CRT)
    jhd_tls_mpi_init( &TP ); jhd_tls_mpi_init( &TQ );
#endif
    ...
    if( f_rng != NULL )
    {
         ...
         * T = T * Vi mod N
         */
        JHD_TLS_MPI_CHK( rsa_prepare_blinding( ctx, f_rng, p_rng ) );
        ...
        JHD_TLS_MPI_CHK( jhd_tls_mpi_add_mpi( &DP_blind, &DP_blind,
                    &ctx->DP ) );
        ...
        JHD_TLS_MPI_CHK( jhd_tls_mpi_add_mpi( &DQ_blind, &DQ_blind,
                    &ctx->DQ ) );
    ...
    JHD_TLS_MPI_CHK( jhd_tls_mpi_exp_mod( &C, &T, &ctx->E,
                                          &ctx->N, &ctx->RN ) );
...
#if !defined(JHD_TLS_RSA_NO_CRT)
    jhd_tls_mpi_free( &TP ); jhd_tls_mpi_free( &TQ );
#endif
...
static int mgf_mask( unsigned char *dst, size_t dlen, unsigned char *src,
                      size_t slen, jhd_tls_md_context_t *md_ctx )
{
    ...
    while( dlen > 0 )
    {
        ...
        if( ( ret = jhd_tls_md_starts( md_ctx ) ) != 0 )
            goto exit;
        if( ( ret = jhd_tls_md_update( md_ctx, src, slen ) ) != 0 )
            goto exit;
        if( ( ret = jhd_tls_md_update( md_ctx, counter, 4 ) ) != 0 )
            goto exit;
        if( ( ret = jhd_tls_md_finish( md_ctx, mask ) ) != 0 )
            goto exit;
...
exit:
    jhd_tls_platform_zeroize( mask, sizeof( mask ) );
                            ...
                            unsigned char *output )
{
    ...
    *p++ = 0;
    ...
    if( ( ret = f_rng( p_rng, p, hlen ) ) != 0 )
        return( JHD_TLS_ERR_RSA_RNG_FAILED + ret );

    p += hlen;

    /* Construct DB */
    if( ( ret = jhd_tls_md( md_info, label, label_len, p ) ) != 0 )
        return( ret );
    p += hlen;
...
exit:
    ...
    return( ( mode == JHD_TLS_RSA_PUBLIC )
            ? jhd_tls_rsa_public(  ctx, output, output )
            : jhd_tls_rsa_private( ctx, f_rng, p_rng, output, output ) );
}
#endif /* JHD_TLS_PKCS1_V21 */
                       ...
                       unsigned char *output )
{
    switch( ctx->padding )
    {
        ...
        case JHD_TLS_RSA_PKCS_V15:
            return jhd_tls_rsa_rsaes_pkcs1_v15_encrypt( ctx, f_rng, p_rng, mode, ilen,
                                                input, output );
#endif

#if defined(JHD_TLS_PKCS1_V21)
        case JHD_TLS_RSA_PKCS_V21:
            return jhd_tls_rsa_rsaes_oaep_encrypt( ctx, f_rng, p_rng, mode, NULL, 0,
                                           ilen, input, output );
#endif
                            ...
                            size_t output_max_len )
{
     ...
     * RSA operation
     */
    ret = ( mode == JHD_TLS_RSA_PUBLIC )
          ? jhd_tls_rsa_public(  ctx, input, buf )
          : jhd_tls_rsa_private( ctx, f_rng, p_rng, input, buf );
    ...
    /* Generate lHash */
    if( ( ret = jhd_tls_md( md_info, label, label_len, lhash ) ) != 0 )
        goto cleanup;
...
#if defined(JHD_TLS_PKCS1_V15)
        case JHD_TLS_RSA_PKCS_V15:
            return jhd_tls_rsa_rsaes_pkcs1_v15_decrypt( ctx, f_rng, p_rng, mode, olen,
                                                input, output, output_max_len );
#endif

#if defined(JHD_TLS_PKCS1_V21)
        case JHD_TLS_RSA_PKCS_V21:
            return jhd_tls_rsa_rsaes_oaep_decrypt( ctx, f_rng, p_rng, mode, NULL, 0,
                                           olen, input, output,
                                           output_max_len );
#endif
                         ...
                         unsigned char *sig )
{
    ...
    *p++ = 0x01;
    ...
    if( ( ret = jhd_tls_md_starts( &md_ctx ) ) != 0 )
        goto exit;
    if( ( ret = jhd_tls_md_update( &md_ctx, p, 8 ) ) != 0 )
        goto exit;
    if( ( ret = jhd_tls_md_update( &md_ctx, hash, hashlen ) ) != 0 )
        goto exit;
    if( ( ret = jhd_tls_md_update( &md_ctx, salt, slen ) ) != 0 )
        goto exit;
    if( ( ret = jhd_tls_md_finish( &md_ctx, p ) ) != 0 )
        goto exit;
...
exit:
    ...
    return( ( mode == JHD_TLS_RSA_PUBLIC )
            ? jhd_tls_rsa_public(  ctx, sig, sig )
            : jhd_tls_rsa_private( ctx, f_rng, p_rng, sig, sig ) );
}
#endif /* JHD_TLS_PKCS1_V21 */
 ...
 *
 */
static int rsa_rsassa_pkcs1_v15_encode( jhd_tls_md_type_t md_alg,
                                        unsigned int hashlen,
                                        const unsigned char *hash,
                                        size_t dst_len,
                                        unsigned char *dst )
                               ...
                               unsigned char *sig )
{
    ...
    sig_try = jhd_tls_calloc( 1, ctx->len );
    if( sig_try == NULL )
        return( JHD_TLS_ERR_MPI_ALLOC_FAILED );

    verif = jhd_tls_calloc( 1, ctx->len );
    if( verif == NULL )
    {
    ...
    JHD_TLS_MPI_CHK( jhd_tls_rsa_private( ctx, f_rng, p_rng, sig, sig_try ) );
...
#if defined(JHD_TLS_PKCS1_V15)
        case JHD_TLS_RSA_PKCS_V15:
            return jhd_tls_rsa_rsassa_pkcs1_v15_sign( ctx, f_rng, p_rng, mode, md_alg,
                                              hashlen, hash, sig );
#endif

#if defined(JHD_TLS_PKCS1_V21)
        case JHD_TLS_RSA_PKCS_V21:
            return jhd_tls_rsa_rsassa_pss_sign( ctx, f_rng, p_rng, mode, md_alg,
                                        hashlen, hash, sig );
#endif
                               ...
                               const unsigned char *sig )
{
    ...
    ret = ( mode == JHD_TLS_RSA_PUBLIC )
          ? jhd_tls_rsa_public(  ctx, sig, buf )
          : jhd_tls_rsa_private( ctx, f_rng, p_rng, sig, buf );
    ...
    if( expected_salt_len != JHD_TLS_RSA_SALT_LEN_ANY &&
     ...
     * Generate H = Hash( M' )
     */
    ret = jhd_tls_md_starts( &md_ctx );
    if ( ret != 0 )
        goto exit;
    ret = jhd_tls_md_update( &md_ctx, zeros, 8 );
    if ( ret != 0 )
        goto exit;
    ret = jhd_tls_md_update( &md_ctx, hash, hashlen );
    if ( ret != 0 )
        goto exit;
    ret = jhd_tls_md_update( &md_ctx, p, observed_salt_len );
    if ( ret != 0 )
        goto exit;
    ret = jhd_tls_md_finish( &md_ctx, result );
    if ( ret != 0 )
        goto exit;
...
int jhd_tls_rsa_rsassa_pss_verify( jhd_tls_rsa_context *ctx,
                           ...
                           const unsigned char *sig )
{
    jhd_tls_md_type_t mgf1_hash_id = ( ctx->hash_id != JHD_TLS_MD_NONE )
                             ? (jhd_tls_md_type_t) ctx->hash_id
                             : md_alg;

    return( jhd_tls_rsa_rsassa_pss_verify_ext( ctx, f_rng, p_rng, mode,
                                       md_alg, hashlen, hash,
                                       mgf1_hash_id, JHD_TLS_RSA_SALT_LEN_ANY,
                                       sig ) );
                                 ...
                                 const unsigned char *sig )
{
    ...
    if( ( encoded          = jhd_tls_calloc( 1, sig_len ) ) == NULL ||
        ( encoded_expected = jhd_tls_calloc( 1, sig_len ) ) == NULL )
    {
    ...
    ret = ( mode == JHD_TLS_RSA_PUBLIC )
          ? jhd_tls_rsa_public(  ctx, sig, encoded )
          : jhd_tls_rsa_private( ctx, f_rng, p_rng, sig, encoded );
    if( ret != 0 )
        goto cleanup;
    ...
    if( ( ret = jhd_tls_safer_memcmp( encoded, encoded_expected,
                                      sig_len ) ) != 0 )
    {
        ret = JHD_TLS_ERR_RSA_VERIFY_FAILED;
        goto cleanup;
...
#if defined(JHD_TLS_PKCS1_V15)
        case JHD_TLS_RSA_PKCS_V15:
            return jhd_tls_rsa_rsassa_pkcs1_v15_verify( ctx, f_rng, p_rng, mode, md_alg,
                                                hashlen, hash, sig );
#endif

#if defined(JHD_TLS_PKCS1_V21)
        case JHD_TLS_RSA_PKCS_V21:
            return jhd_tls_rsa_rsassa_pss_verify( ctx, f_rng, p_rng, mode, md_alg,
                                          hashlen, hash, sig );
#endif
...
{
    jhd_tls_mpi_free( &ctx->Vi ); jhd_tls_mpi_free( &ctx->Vf );
    jhd_tls_mpi_free( &ctx->RN ); jhd_tls_mpi_free( &ctx->D  );
    jhd_tls_mpi_free( &ctx->Q  ); jhd_tls_mpi_free( &ctx->P  );
    jhd_tls_mpi_free( &ctx->E  ); jhd_tls_mpi_free( &ctx->N  );

#if !defined(JHD_TLS_RSA_NO_CRT)
    jhd_tls_mpi_free( &ctx->RQ ); jhd_tls_mpi_free( &ctx->RP );
    jhd_tls_mpi_free( &ctx->QP ); jhd_tls_mpi_free( &ctx->DQ );
    jhd_tls_mpi_free( &ctx->DP );
#endif /* JHD_TLS_RSA_NO_CRT */

#if defined(JHD_TLS_THREADING_C)
    jhd_tls_mutex_free( &ctx->mutex );
#endif
}
...
int jhd_tls_rsa_self_test( int verbose )
{
    ...
    jhd_tls_mpi_init( &K );
    jhd_tls_rsa_init( &rsa, JHD_TLS_RSA_PKCS_V15, 0 );

    JHD_TLS_MPI_CHK( jhd_tls_mpi_read_string( &K, 16, RSA_N  ) );
    JHD_TLS_MPI_CHK( jhd_tls_rsa_import( &rsa, &K, NULL, NULL, NULL, NULL ) );
    JHD_TLS_MPI_CHK( jhd_tls_mpi_read_string( &K, 16, RSA_P  ) );
    JHD_TLS_MPI_CHK( jhd_tls_rsa_import( &rsa, NULL, &K, NULL, NULL, NULL ) );
    JHD_TLS_MPI_CHK( jhd_tls_mpi_read_string( &K, 16, RSA_Q  ) );
    JHD_TLS_MPI_CHK( jhd_tls_rsa_import( &rsa, NULL, NULL, &K, NULL, NULL ) );
    JHD_TLS_MPI_CHK( jhd_tls_mpi_read_string( &K, 16, RSA_D  ) );
    JHD_TLS_MPI_CHK( jhd_tls_rsa_import( &rsa, NULL, NULL, NULL, &K, NULL ) );
    JHD_TLS_MPI_CHK( jhd_tls_mpi_read_string( &K, 16, RSA_E  ) );
    JHD_TLS_MPI_CHK( jhd_tls_rsa_import( &rsa, NULL, NULL, NULL, NULL, &K ) );

    JHD_TLS_MPI_CHK( jhd_tls_rsa_complete( &rsa ) );

    if( verbose != 0 )
        jhd_tls_printf( "  RSA key validation: " );

    if( jhd_tls_rsa_check_pubkey(  &rsa ) != 0 ||
        jhd_tls_rsa_check_privkey( &rsa ) != 0 )
    {
        if( verbose != 0 )
    ...
    if( jhd_tls_rsa_pkcs1_encrypt( &rsa, myrand, NULL, JHD_TLS_RSA_PUBLIC,
    ...
    if( jhd_tls_rsa_pkcs1_decrypt( &rsa, myrand, NULL, JHD_TLS_RSA_PRIVATE,
...
#if defined(JHD_TLS_SHA1_C)
    if( verbose != 0 )
        jhd_tls_printf( "  PKCS#1 data sign  : " );

    if( jhd_tls_sha1_ret( rsa_plaintext, PT_LEN, sha1sum ) != 0 )
    {
        if( verbose != 0 )
            jhd_tls_printf( "failed\n" );
    ...
    if( jhd_tls_rsa_pkcs1_sign( &rsa, myrand, NULL,
                                JHD_TLS_RSA_PRIVATE, JHD_TLS_MD_SHA1, 0,
    ...
    if( jhd_tls_rsa_pkcs1_verify( &rsa, NULL, NULL,
                                  JHD_TLS_RSA_PUBLIC, JHD_TLS_MD_SHA1, 0,
                                  sha1sum, rsa_ciphertext ) != 0 )
    {
        if( verbose != 0 )
            jhd_tls_printf( "failed\n" );
