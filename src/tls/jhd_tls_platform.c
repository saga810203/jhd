...
int jhd_tls_platform_std_nv_seed_read( unsigned char *buf, size_t buf_len )
{
    ...
    if( ( file = fopen( JHD_TLS_PLATFORM_STD_NV_SEED_FILE, "rb" ) ) == NULL )
        return( -1 );

    if( ( n = fread( buf, 1, buf_len, file ) ) != buf_len )
    {
        fclose( file );
        jhd_tls_platform_zeroize( buf, buf_len );
        return( -1 );
    }
...
int jhd_tls_platform_std_nv_seed_write( unsigned char *buf, size_t buf_len )
{
    ...
    if( ( file = fopen( JHD_TLS_PLATFORM_STD_NV_SEED_FILE, "w" ) ) == NULL )
        return -1;
...
#define JHD_TLS_PLATFORM_STD_NV_SEED_READ   platform_nv_seed_read_uninit
#endif /* !JHD_TLS_PLATFORM_STD_NV_SEED_READ */
...
#define JHD_TLS_PLATFORM_STD_NV_SEED_WRITE   platform_nv_seed_write_uninit
#endif /* !JHD_TLS_PLATFORM_STD_NV_SEED_WRITE */

int (*jhd_tls_nv_seed_read)( unsigned char *buf, size_t buf_len ) =
            JHD_TLS_PLATFORM_STD_NV_SEED_READ;
int (*jhd_tls_nv_seed_write)( unsigned char *buf, size_t buf_len ) =
            JHD_TLS_PLATFORM_STD_NV_SEED_WRITE;

int jhd_tls_platform_set_nv_seed(
        int (*nv_seed_read_func)( unsigned char *buf, size_t buf_len ),
        int (*nv_seed_write_func)( unsigned char *buf, size_t buf_len ) )
{
    jhd_tls_nv_seed_read = nv_seed_read_func;
    jhd_tls_nv_seed_write = nv_seed_write_func;
    return( 0 );
}
