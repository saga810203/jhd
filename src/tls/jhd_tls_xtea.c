...
{
    memset( ctx, 0, sizeof( jhd_tls_xtea_context ) );
}
...
void jhd_tls_xtea_free( jhd_tls_xtea_context *ctx )
{
    ...
    jhd_tls_platform_zeroize( ctx, sizeof( jhd_tls_xtea_context ) );
}
 ...
 */
int jhd_tls_xtea_crypt_ecb( jhd_tls_xtea_context *ctx, int mode,
 ...
 */
int jhd_tls_xtea_crypt_cbc( jhd_tls_xtea_context *ctx, int mode, size_t length,
                    ...
                    unsigned char *output)
{
    ...
    if( length % 8 )
        return( JHD_TLS_ERR_XTEA_INVALID_INPUT_LENGTH );

    if( mode == JHD_TLS_XTEA_DECRYPT )
    {
        while( length > 0 )
        {
            memcpy( temp, input, 8 );
            jhd_tls_xtea_crypt_ecb( ctx, mode, input, output );
    ...
    else
    {
        while( length > 0 )
        {
            ...
            jhd_tls_xtea_crypt_ecb( ctx, mode, output, output );
            memcpy( iv, output, 8 );
...
int jhd_tls_xtea_self_test( int verbose )
{
    int i, ret = 0;
    unsigned char buf[8];
    jhd_tls_xtea_context ctx;

    jhd_tls_xtea_init( &ctx );
    for( i = 0; i < 6; i++ )
    {
        if( verbose != 0 )
            jhd_tls_printf( "  XTEA test #%d: ", i + 1 );

        memcpy( buf, xtea_test_pt[i], 8 );

        jhd_tls_xtea_setup( &ctx, xtea_test_key[i] );
        jhd_tls_xtea_crypt_ecb( &ctx, JHD_TLS_XTEA_ENCRYPT, buf, buf );
