 ...
 */
int jhd_tls_ctr_drbg_seed_entropy_len(
                   jhd_tls_ctr_drbg_context *ctx,
                   ...
                   size_t entropy_len )
{
    ...
    memset( key, 0, JHD_TLS_CTR_DRBG_KEYSIZE );

    jhd_tls_aes_init( &ctx->aes_ctx );
...
int jhd_tls_ctr_drbg_seed( jhd_tls_ctr_drbg_context *ctx,
                   ...
                   size_t len )
{
    return( jhd_tls_ctr_drbg_seed_entropy_len( ctx, f_entropy, p_entropy, custom, len,
                            ...
                            const unsigned char *data, size_t data_len )
{
    ...
    memset( buf, 0, JHD_TLS_CTR_DRBG_MAX_SEED_INPUT + JHD_TLS_CTR_DRBG_BLOCKSIZE + 16 );
    ...
    *p++ = JHD_TLS_CTR_DRBG_SEEDLEN;
    ...
    for( j = 0; j < JHD_TLS_CTR_DRBG_SEEDLEN; j += JHD_TLS_CTR_DRBG_BLOCKSIZE )
    {
        ...
        while( use_len > 0 )
        {
            ...
            use_len -= ( use_len >= JHD_TLS_CTR_DRBG_BLOCKSIZE ) ?
                       JHD_TLS_CTR_DRBG_BLOCKSIZE : use_len;

            if( ( ret = jhd_tls_aes_crypt_ecb( &aes_ctx, JHD_TLS_AES_ENCRYPT, chain, chain ) ) != 0 )
            {
                goto exit;
            }
        }
    ...
    for( j = 0; j < JHD_TLS_CTR_DRBG_SEEDLEN; j += JHD_TLS_CTR_DRBG_BLOCKSIZE )
    {
        if( ( ret = jhd_tls_aes_crypt_ecb( &aes_ctx, JHD_TLS_AES_ENCRYPT, iv, iv ) ) != 0 )
        {
            goto exit;
        }
...
static int ctr_drbg_update_internal( jhd_tls_ctr_drbg_context *ctx,
                              const unsigned char data[JHD_TLS_CTR_DRBG_SEEDLEN] )
{
    unsigned char tmp[JHD_TLS_CTR_DRBG_SEEDLEN];
    unsigned char *p = tmp;
    int i, j;
    ...
    for( j = 0; j < JHD_TLS_CTR_DRBG_SEEDLEN; j += JHD_TLS_CTR_DRBG_BLOCKSIZE )
    {
         ...
         * Crypt counter block
         */
        if( ( ret = jhd_tls_aes_crypt_ecb( &ctx->aes_ctx, JHD_TLS_AES_ENCRYPT, ctx->counter, p ) ) != 0 )
        {
            return( ret );
        }
                     ...
                     const unsigned char *additional, size_t len )
{
    ...
    if( ctx->entropy_len > JHD_TLS_CTR_DRBG_MAX_SEED_INPUT ||
        len > JHD_TLS_CTR_DRBG_MAX_SEED_INPUT - ctx->entropy_len )
        return( JHD_TLS_ERR_CTR_DRBG_INPUT_TOO_BIG );
                              ...
                              const unsigned char *additional, size_t add_len )
{
    ...
    while( output_len > 0 )
    {
         ...
         * Crypt counter block
         */
        if( ( ret = jhd_tls_aes_crypt_ecb( &ctx->aes_ctx, JHD_TLS_AES_ENCRYPT, ctx->counter, tmp ) ) != 0 )
        {
            return( ret );
        }

        use_len = ( output_len > JHD_TLS_CTR_DRBG_BLOCKSIZE ) ? JHD_TLS_CTR_DRBG_BLOCKSIZE :
                                                       output_len;
        /*
...
int jhd_tls_ctr_drbg_self_test( int verbose )
{
    ...
    jhd_tls_ctr_drbg_init( &ctx );

    /*
     * Based on a NIST CTR_DRBG test vector (PR = True)
     */
    if( verbose != 0 )
        jhd_tls_printf( "  CTR_DRBG (PR = TRUE) : " );

    test_offset = 0;
    CHK( jhd_tls_ctr_drbg_seed_entropy_len( &ctx, ctr_drbg_self_test_entropy,
                                (void *) entropy_source_pr, nonce_pers_pr, 16, 32 ) );
    jhd_tls_ctr_drbg_set_prediction_resistance( &ctx, JHD_TLS_CTR_DRBG_PR_ON );
    CHK( jhd_tls_ctr_drbg_random( &ctx, buf, JHD_TLS_CTR_DRBG_BLOCKSIZE ) );
    CHK( jhd_tls_ctr_drbg_random( &ctx, buf, JHD_TLS_CTR_DRBG_BLOCKSIZE ) );
    CHK( memcmp( buf, result_pr, JHD_TLS_CTR_DRBG_BLOCKSIZE ) );

    jhd_tls_ctr_drbg_free( &ctx );
     ...
     */
    if( verbose != 0 )
        jhd_tls_printf( "  CTR_DRBG (PR = FALSE): " );
    ...
    CHK( jhd_tls_ctr_drbg_random( &ctx, buf, 16 ) );
    CHK( jhd_tls_ctr_drbg_reseed( &ctx, NULL, 0 ) );
    CHK( jhd_tls_ctr_drbg_random( &ctx, buf, 16 ) );
