...
typedef enum
{
    JHD_TLS_PK_DEBUG_NONE = 0,
    JHD_TLS_PK_DEBUG_MPI,
    JHD_TLS_PK_DEBUG_ECP,
 ...
 */
static inline jhd_tls_rsa_context *jhd_tls_pk_rsa( const jhd_tls_pk_context pk )
{
    return( (jhd_tls_rsa_context *) (pk).pk_ctx );
}
#endif /* JHD_TLS_RSA_C */
 ...
 */
typedef int (*jhd_tls_pk_rsa_alt_decrypt_func)( void *ctx, int mode, size_t *olen,
                    const unsigned char *input, unsigned char *output,
                    size_t output_max_len );
typedef int (*jhd_tls_pk_rsa_alt_sign_func)( void *ctx,
                    int (*f_rng)(void *, unsigned char *, size_t), void *p_rng,
                    int mode, jhd_tls_md_type_t md_alg, unsigned int hashlen,
                    const unsigned char *hash, unsigned char *sig );
typedef size_t (*jhd_tls_pk_rsa_alt_key_len_func)( void *ctx );
...
/**
 * \brief           Free a jhd_tls_pk_context
 */
 ...
 *
 * \return          0 on success,
 *                  JHD_TLS_ERR_PK_BAD_INPUT_DATA on invalid input,
 *                  JHD_TLS_ERR_PK_ALLOC_FAILED on allocation failure.
 *
 * \note            For contexts holding an RSA-alt key, use
 *                  \c jhd_tls_pk_setup_rsa_alt() instead.
 */
 ...
 *
 * \return          0 on success, or JHD_TLS_ERR_PK_BAD_INPUT_DATA if the
 *                  context wasn't already initialized as RSA_ALT.
 *
 * \note            This function replaces \c jhd_tls_pk_setup() for RSA-alt.
 */
 ...
 * \param ctx       Context to use
 *
 * \return          Type on success, or JHD_TLS_PK_NONE
 */
 ...
 */
int jhd_tls_pk_parse_key( jhd_tls_pk_context *ctx,
