...
typedef struct {
	...
	jhd_tls_mpi private_key; /*!<  our secret value                  */
	jhd_tls_ecp_point public_key; /*!<  our public value                  */
} jhd_tls_ecp_keypair;
 ...
 */
const jhd_tls_ecp_curve_info *jhd_tls_ecp_curve_list();


#else
#define jhd_tls_ecp_curve_list()    ecp_supported_curves
#endif


#ifdef JHD_LOG_LEVEL_INFO
int jhd_tls_ecp_keypair_equals(jhd_tls_ecp_keypair *key1,jhd_tls_ecp_keypair *key2);
int jhd_tls_ecp_point_equals(jhd_tls_ecp_point *key1,jhd_tls_ecp_point *key2);
#endif
...
#else
#define jhd_tls_ecp_is_zero(pt) (jhd_tls_mpi_cmp_int(&((pt)->Z), 0) == 0)
 ...
 */
int jhd_tls_ecp_point_write_binary(const jhd_tls_ecp_group *grp, const jhd_tls_ecp_point *P, int format, size_t *olen, unsigned char *buf, size_t buflen);
 ...
 *
 */
int jhd_tls_ecp_point_read_binary(const jhd_tls_ecp_group *grp, jhd_tls_ecp_point *P, const unsigned char *buf, size_t ilen);
 ...
 * \return          #JHD_TLS_ERR_ECP_BAD_INPUT_DATA if input is invalid.
 */
int jhd_tls_ecp_tls_read_point(const jhd_tls_ecp_group *grp, jhd_tls_ecp_point *pt, const unsigned char **buf, size_t len);
 ...
 * \return          #JHD_TLS_ERR_MPI_ALLOC_FAILED on memory-allocation failure.
 */
int jhd_tls_ecp_mul(jhd_tls_ecp_group *grp, jhd_tls_ecp_point *R, const jhd_tls_mpi *m, const jhd_tls_ecp_point *P);
int jhd_tls_ecp_mul_specific(jhd_tls_ecp_group *grp, jhd_tls_ecp_point *R, const jhd_tls_mpi *m, const jhd_tls_ecp_point *P,void (*f_rng)(void *, unsigned char *, size_t), void *p_rng);
/**
 ...
 *                  on failure.
 */
int jhd_tls_ecp_gen_keypair_base(jhd_tls_ecp_group *grp, const jhd_tls_ecp_point *G, jhd_tls_mpi *private_key, jhd_tls_ecp_point *public_key);
/**
...
#define jhd_tls_ecp_gen_keypair(grp,d,Q) jhd_tls_ecp_gen_keypair_base(grp, &(((jhd_tls_ecp_group *)(grp))->G), d, Q)
...
#ifdef JHD_TLS_INLINE
extern jhd_tls_ecp_curve_info ecp_supported_curves[];
#endif
