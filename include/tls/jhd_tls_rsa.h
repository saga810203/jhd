...
#define JHD_TLS_RSA_SERIALZA_LEN  8000
...
typedef struct {
		...
		size_t len;
		jhd_tls_md_info_t *md_info;
		char serializa_context[JHD_TLS_RSA_SERIALZA_LEN];
} jhd_tls_serializa_rsa_context;
...
#else
#define jhd_tls_rsa_get_len(ctx)  ((ctx)->len)
#endif
 ...
 * \return         An \c JHD_TLS_ERR_RSA_XXX error code on failure.
 */
int jhd_tls_rsa_rsassa_pss_sign(jhd_tls_rsa_context *ctx,  int mode, const jhd_tls_md_info_t *md_info, unsigned int hashlen, const unsigned char *hash,
        unsigned char *sig);
