...
#else
#define JHD_TLS_MD_MAX_SIZE         32  /* longest known is SHA256 or less */
#endif
 ...
 *                  message-digest enumeration #jhd_tls_md_type_t.
 *                  The last entry is 0.
 */
 ...
 *
 *                  It should be called after jhd_tls_md_init() or jhd_tls_md_free().
 *                  Makes it necessary to call jhd_tls_md_free() later.
 *
 * \deprecated      Superseded by jhd_tls_md_setup() in 2.0.0
 *
 ...
 */
int jhd_tls_md_init_ctx( jhd_tls_md_context_t *ctx, const jhd_tls_md_info_t *md_info ) JHD_TLS_DEPRECATED;
 ...
 *                  context.
 *
 * \note            You must call jhd_tls_md_setup() on \c dst before calling
 *                  this function.
 *
 ...
 *
 * \return          \c 0 on success.
 * \return          #JHD_TLS_ERR_MD_BAD_INPUT_DATA on parameter-verification failure.
 */
 ...
 *
 * \return         \c 0 on success.
 * \return         #JHD_TLS_ERR_MD_FILE_IO_ERROR on an I/O error accessing
 *                 the file pointed by \p path.
 * \return         #JHD_TLS_ERR_MD_BAD_INPUT_DATA if \p md_info was NULL.
 */
int jhd_tls_md_file( const jhd_tls_md_info_t *md_info, const char *path,
                     unsigned char *output );
#endif /* JHD_TLS_FS_IO */
 ...
 *                  failure.
 */
int jhd_tls_md_hmac_starts( jhd_tls_md_context_t *ctx, const unsigned char *key,
                    size_t keylen );
 ...
 *                  failure.
 */
int jhd_tls_md_hmac_update( jhd_tls_md_context_t *ctx, const unsigned char *input,
                    size_t ilen );
 ...
 *                 failure.
 */
int jhd_tls_md_hmac( const jhd_tls_md_info_t *md_info, const unsigned char *key, size_t keylen,
